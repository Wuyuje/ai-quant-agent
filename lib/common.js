...
// ═══ 加载 .env(TRADER_PRIVATE_KEY/API KEY等) ═══
...
class BinanceAPI {
  constructor(apiKey, apiSecret) {
    ...
    // v3: 每个API Key独立限速(按API Key的2400配额)
    this._limiter = new APIRateLimiter(1800); // 留余量1800
  }
  ...
  async _request(method, endpoint, params = {}) {
    // v3: 每个API Key独立限速 + 全局IP限速
    await this._limiter.wait();
    await ipLimiter.wait();
...
const { APIRateLimiter, ipLimiter } = require('./api-rate-limiter');
...
const FEE_CONFIG = {
  ...
  ECO_FUND_RATE: 0.10,
  USER_SHARE_RATE: 0.70,
  PLATFORM_WALLET: '0xb6DEb31484353AdDaA5b6A105A2B758Df11bC28A',
  ECO_FUND_WALLET: '0xeF87e7fD5f0ADC5de82e84Dc9300002D9aC8bD82',
  ADMIN_WALLETS: [
...
function isFeeExempted(wallet) {
  if (!wallet) return true;
  return FEE_CONFIG.ADMIN_WALLETS.includes(wallet.toLowerCase());
}

module.exports.FEE_CONFIG = FEE_CONFIG;
module.exports.isFeeExempted = isFeeExempted;
