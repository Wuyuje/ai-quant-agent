...
const { globalLimiter } = require('./api-rate-limiter');
const fs = require('fs');
...
const CONFIG = {
  ...
  // v3: 反向对冲 — 浮亏≥15%时开反向仓对冲
  hedgeTriggerPct: 15,      // 浮亏≥15%含杠杆(价格跌5%)触发对冲
  hedgeRatio: 0.5,          // 对冲仓位=原仓位50%
  hedgeClosePct: 30,        // 原仓位浮亏≥30%时全平+平对冲仓
...
class BBStrategy {
    ...
    try {
      ...
      for (const sym of Object.keys(this.positions)) {
        if (!remoteSymbols.has(sym)) {
          this._log(`📌 ${sym} 远程已平仓, 记录并清除本地状态`);
          // v3: 记录远程平仓的交易历史
          const closedPos = this.positions[sym];
          if (closedPos) {
            const exitPrice = closedPos.currentPrice || closedPos.entryPrice;
            const pnlUsd = this._calcPnlUsd(closedPos, exitPrice);
            const pnlPct = this._calcPnlPct(closedPos, exitPrice);
  ...
  // ═══ 平仓执行 ═══
  // v3: 平对冲仓
  async _closeHedge(symbol, pos) {
    try {
      if (pos.side === 'LONG') await this.api.closeShort(symbol, pos._hedgeQty, this.precisionMap);
      else await this.api.closeLong(symbol, pos._hedgeQty, this.precisionMap);
      this._log(`🛡️ ${symbol} 对冲仓已平`);
    } catch(e) { this._log(`⚠️ ${symbol} 对冲平仓失败: ${e.message}`); }
  }
  ...
  async _scan() {
    ...
    for (const symbol of Object.keys(this.positions)) {
      ...
      try {
        ...
        // v3: 反向对冲 — 浮亏≥15%时开反向仓
        if (!pos._hedged && totalLoss >= CONFIG.hedgeTriggerPct && totalLoss < CONFIG.hedgeClosePct) {
          try {
            const hedgeDir = pos.side === 'LONG' ? 'SHORT' : 'LONG';
            const hedgeQty = pos.qty * CONFIG.hedgeRatio;
            if (pos.side === 'LONG') await this.api.marketShort(symbol, hedgeQty, pos.leverage, this.precisionMap, 0);
            else await this.api.marketLong(symbol, hedgeQty, pos.leverage, this.precisionMap, 0);
            pos._hedged = true;
            pos._hedgeQty = hedgeQty;
            this._log(`🛡️ ${symbol} 对冲开仓 ${hedgeDir} qty=${hedgeQty.toFixed(4)} (原${pos.side}浮亏${totalLoss.toFixed(1)}%)`);
          } catch(e) { this._log(`⚠️ ${symbol} 对冲开仓失败: ${e.message}`); }
        }

        // 兜底: 总浮亏≥30%时全平+平对冲仓
        if (totalLoss >= CONFIG.hedgeClosePct) {
          if (pos._hedged) { await this._closeHedge(symbol, pos); }
          this._log(`🔴 ${symbol} 总浮亏止损: ${totalLoss.toFixed(1)}%≥${CONFIG.hedgeClosePct}% (含对冲平仓)`);
          await this._closePosition(symbol, pos, `总浮亏止损: ${totalLoss.toFixed(1)}%≥${CONFIG.hedgeClosePct}%`);
          continue;
        }
