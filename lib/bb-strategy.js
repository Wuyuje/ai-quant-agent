...
const CONFIG = {
  ...
  // v3: 减仓+反向开仓 (去除对冲)
  reduceLossPct: 10,        // 浮亏≥10%含杠杆(价格跌3.3%)减仓50%+开反向新仓
...
class BBStrategy {
  ...
  // ═══ 平仓执行 ═══
  // v3: 减仓50%
  async _reducePosition(symbol, pos) {
    try {
      const reduceQty = pos.qty * 0.5;
      if (pos.side === 'LONG') await this.api.closeLong(symbol, reduceQty, this.precisionMap);
      else await this.api.closeShort(symbol, reduceQty, this.precisionMap);
      pos.qty = pos.qty - reduceQty;
      pos._reduced = true;
      this._log(`📉 ${symbol} 减仓50% qty=${reduceQty.toFixed(4)}→剩余${pos.qty.toFixed(4)}`);
    } catch(e) { this._log(`⚠️ ${symbol} 减仓失败: ${e.message}`); }
  }

  // v3: 开反向新仓50%
  async _openReverse(symbol, pos) {
    try {
      const reverseDir = pos.side === 'LONG' ? 'SHORT' : 'LONG';
      const reverseQty = pos.qty * 0.5;
      if (pos.side === 'LONG') await this.api.marketShort(symbol, reverseQty, pos.leverage, this.precisionMap, 0);
      else await this.api.marketLong(symbol, reverseQty, pos.leverage, this.precisionMap, 0);
      this._log(`🔄 ${symbol} 反向开仓 ${reverseDir} qty=${reverseQty.toFixed(4)}`);
    } catch(e) { this._log(`⚠️ ${symbol} 反向开仓失败: ${e.message}`); }
  }

  // v3: 获取Binance实际持仓数
  async _getBinanceCount() {
    try {
      const remote = await Promise.race([this.api.getPositions(), new Promise((_,rej) => setTimeout(() => rej(new Error('timeout')), 8000))]).catch(e => null);
      if (remote) return remote.filter(p => Math.abs(parseFloat(p.positionAmt)) > 0).length;
      return Object.keys(this.positions).length;
    } catch(e) { return Object.keys(this.positions).length; }
  }
  ...
  async _scan() {
    ...
    for (const symbol of Object.keys(this.positions)) {
      ...
      try {
        ...
        // v3: 浮亏≥10%减仓50%+开反向新仓
        if (!pos._reduced && totalLoss >= CONFIG.reduceLossPct && totalLoss < CONFIG.fullStopPct) {
          await this._reducePosition(symbol, pos);
          // 用释放的保证金开反向新仓(仓位不满时)
          const binanceCount = await this._getBinanceCount();
          if (binanceCount < 10) {
            await this._openReverse(symbol, pos);
          }
        }

        // 浮亏≥20%原仓全平(反向仓继续持有)
        if (totalLoss >= CONFIG.fullStopPct) {
          this._log(`🔴 ${symbol} 总浮亏止损: ${totalLoss.toFixed(1)}%≥${CONFIG.fullStopPct}% 原仓全平,反向仓继续`);
          await this._closePosition(symbol, pos, `总浮亏止损: ${totalLoss.toFixed(1)}%≥${CONFIG.fullStopPct}%`);
          continue;
        }
