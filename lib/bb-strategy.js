...
const CONFIG = {
  ...
  atrPeriod: 14,
  minAtrPct: 0.10,
  atrStopMultiplier: 10,    // 允许10个ATR反向偏移才止损(趋势给空间,避免过紧误杀)
  ...
  // ═══ 增强风控配置(修复缺失,接入止损) ═══
  consecutiveLossPct: 2.5,      // 连续K线偏离开仓价≥2.5%(不含杠杆)才触发连续止损
  consecutiveLossKlines: 10,    // 连续10根K线都在反方向
  noReplenishLossPct: 10,       // 浮亏≥10%含杠杆时不补仓
  liquidationWarnPct: 0.80,     // 价格走到强平距离80%时紧急止损
  maxHoldHours: 4,              // 持仓超4小时触发时间止损判断
  maxHoldLossPct: 5,            // 持仓>4h且浮亏≥5%含杠杆则时间止损
  avgPriceLossPct: 8,           // 补仓后价格偏离均价≥8%(不含杠杆)则均价止损
...
class BBStrategy {
  ...
  // ═══ ATR止损 ═══
  // ═══ ATR止损：价格反向偏离开仓价超过 N×ATR 才触发 ═══
  // (原实现 stopPct=ATR%×2 太紧=0.4%，会误杀正常持仓)
  // N: 允许N个ATR的合理波动(趋势1-3个ATR移动,给空间)
  checkAtrStopLoss(klines, pos) {
    const atr = Indicators.atr(klines, CONFIG.atrPeriod);
    if (!atr) return { action: 'HOLD' };
    const entry = pos.entryPrice;
    const price = pos.currentPrice || pos.entryPrice;
    const atrDist = atr * CONFIG.atrStopMultiplier; // 允许 N×ATR 反向偏移
    if (pos.side === 'LONG') {
      // 多单: 价格跌低于 开仓价 - N×ATR 才止损
      if (entry - price >= atrDist) return { action: 'CLOSE', reason: `ATR止损: 价格${price.toFixed(4)}<开仓${entry.toFixed(4)}-${CONFIG.atrStopMultiplier}ATR` };
    } else {
      // 空单: 价格涨高于 开仓价 + N×ATR 才止损
      if (price - entry >= atrDist) return { action: 'CLOSE', reason: `ATR止损: 价格${price.toFixed(4)}>开仓${entry.toFixed(4)}+${CONFIG.atrStopMultiplier}ATR` };
    }
    return { action: 'HOLD' };
  }
        ...
        if (!remoteSymbols.has(sym)) {
          ...
          if (closedPos) {
            ...
            try {
              ...
              const symIncome = (income || []).filter(r => r.symbol === sym && Math.abs(parseFloat(r.income)) > 0);
              if (symIncome.length > 0) {
                // ✓ 有币安真实已实现盈亏 → 记录真实平仓
                pnlUsd = symIncome.reduce((s, r) => s + parseFloat(r.income), 0);
                this._log(`📊 ${sym} Binance真实平仓PnL=$${pnlUsd.toFixed(4)} (${symIncome.length}笔)`);
                this._recordTrade(sym, closedPos, '真实平仓', pnlUsd);
                ...
                // ✗ 币安无真实平仓记录(可能只是API拉取失败/非真实成交) → 不写仪表盘,避免虚账
                this._log(`⚠️ ${sym} 币安无真实平仓记录,跳过(不记录虚账)`);
              }
            } catch(e) {
              this._log(`⚠️ ${sym} 拉取平仓PnL失败,跳过(不记录虚账): ${e.message.slice(0,30)}`);
            }
          }
  ...
  async _openPosition(symbol, direction, klines) {
    ...
    let margin = Math.min(this.balance * positionPct, 30); // v3: 单仓上限$30

    // v3.1: 账户级总风控 — 防多引擎叠加超载/防强平
    if (this.accountGuard) {
      ...
      if (!cap.allowed) {
        this._log(`⏭️ ${symbol} 账户容量不足,不开仓: ${cap.reason}`);
        return;
      }
      // 若可开,再按剩余容量进一步限制单仓量
      if (cap.limitNotional < notional) {
        margin = Math.max(0, cap.limitNotional / CONFIG.leverage);
        if (margin < 1) { this._log(`⏭️ ${symbol} 账户剩余容量过小,不开仓`); return; }
      }
    }
  ...
  async _syncIncomeHistory() {
    try {
      ...
      if (added > 0) {
        if (trades.length > 20) trades = trades.slice(-20);
  ...
  async _replenishPosition(symbol, pos, amount) {
    try {
      // v3.1: 补仓前也做账户容量检查(补仓是仓位膨胀主因,防超载/强平)
      if (this.accountGuard) {
        const addNotional = amount * CONFIG.leverage;
        const cap = await this.accountGuard.checkCanOpen(this.api, addNotional, symbol);
        if (!cap.allowed) { this._log(`⏭️ ${symbol} 补仓取消(账户容量不足): ${cap.reason}`); return; }
      }
  ...
  async _scan() {
    ...
    for (const symbol of Object.keys(this.positions)) {
      ...
      try {
        ...
        // v3: 单向模式 — 浮亏≥15%直接止损平仓(不反向开仓，避免逆变追仓)
        if (totalLoss >= CONFIG.reverseTriggerPct) {
          this._log(`🔴 ${symbol} 浮亏止损: ${totalLoss.toFixed(1)}%≥${CONFIG.reverseTriggerPct}%`);
          await this._closePosition(symbol, pos, '浮亏止损');
          continue;
        }
        ...
        // ═══ 增强风控(修复：接入此前未启用的止损) ═══
        // ATR止损 — 价格偏离ATR合理范围
        const atrSl = this.checkAtrStopLoss(klines, pos);
        if (atrSl.action === 'CLOSE') { this._log(`🔴 ${symbol} ${atrSl.reason}`); await this._closePosition(symbol, pos, atrSl.reason); continue; }
        // 总浮亏兜底
        const totSl = this.checkTotalLossStop(pos);
        if (totSl.action === 'CLOSE') { this._log(`🔴 ${symbol} ${totSl.reason}`); await this._closePosition(symbol, pos, totSl.reason); continue; }
        // 强平预警 — 防爆仓
        const liq = this.checkLiquidationWarning(pos);
        if (liq.action === 'CLOSE') { this._log(`🔴 ${symbol} ${liq.reason}`); await this._closePosition(symbol, pos, liq.reason); continue; }
        // 连续反向K线止损
        const cl = this.checkConsecutiveLoss(klines, pos);
        if (cl.action === 'CLOSE') { this._log(`🔴 ${symbol} ${cl.reason}`); await this._closePosition(symbol, pos, cl.reason); continue; }
        // 时间止损
        const ts = this.checkTimeStop(pos);
        if (ts.action === 'CLOSE') { this._log(`🔴 ${symbol} ${ts.reason}`); await this._closePosition(symbol, pos, ts.reason); continue; }
        // 补仓均价止损
        const avgSl = this.checkAvgPriceStop(pos);
        if (avgSl.action === 'CLOSE') { this._log(`🔴 ${symbol} ${avgSl.reason}`); await this._closePosition(symbol, pos, avgSl.reason); continue; }
        } // end grace period
