...
class BBStrategy {
  ...
  async _openPosition(symbol, direction, klines) {
    ...
    const trendCount = Object.keys(this._trendPositions || {}).length;
    const totalCount = myCount + trendCount;
    const myMax = this.getDynamicMaxPositions();
    if (myCount >= myMax || totalCount >= 10) { this._log(`⏭️ ${symbol} 持仓已满(我的${myCount}/${myMax} 总${totalCount}/10)`); return; }
  ...
  async _scan() {
    ...
    // 开新仓 — 有交易结束自动选币补充满足5仓
    if (Object.keys(this.positions).length >= this.getDynamicMaxPositions()) return;
