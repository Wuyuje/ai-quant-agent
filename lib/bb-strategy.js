      ...
      for (const sym of Object.keys(this.positions)) {
        if (!remoteSymbols.has(sym)) {
          ...
          if (closedPos) {
            ...
            this._recordTrade(sym, closedPos, iReason, pnlUsd);
            // 远程已平仓也补扣算力费(盈利才扣),与主动平仓一致,避免漏扣
            try { await this._collectServiceFee(sym, pnlUsd); } catch(e) { this._log(`⚠️ ${sym} 远程已平仓扣费失败: ${e.message.slice(0,30)}`); }
  ...
  async _closeAndReverse(symbol, pos) {
    try {
      ...
      this.positions[symbol] = {
        ...
        replenishCount: 3, mode: pos.mode || '轨道',
