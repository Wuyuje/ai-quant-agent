...
const CONFIG = {
  ...
  singleKLossPct: 20,        // 单K浮亏≥本金20%止损(含杠杆)
  ultimateLossPct: 70,       // 3次补完+总浮亏≥70%终极止损
  
  // v2: 6个防深套补救措施
  totalLossPct: 30,         // 1.总浮亏≥30%含杠杆(价格跌10%)就止损,不要求补完
  consecutiveLossKlines: 5, // 2.连续5根5min K线收盘价在开仓价反方向→止损
  noReplenishLossPct: 10,   // 3.浮亏≥10%含杠杆(价格跌3.3%)时不补仓
  liquidationWarnPct: 0.8,  // 4.价格接近强平价80%时紧急止损
  maxHoldHours: 4,          // 5.持仓>4小时且浮亏>5%→止损
  maxHoldLossPct: 5,        //    时间止损的浮亏门槛
  avgPriceLossPct: 5,       // 6.补仓后价格跌破均价5%→止损
...
class BBStrategy {
  ...
  checkConsecutiveLoss(klines, pos) {
    ...
    if (pos.side === 'LONG') {
      // 做多: 连续N根收盘价都在开仓价下方
      const allBelow = recent.every(k => k.close < pos.entryPrice);
      if (allBelow) return { action: 'CLOSE', reason: `连续${n}根K线收盘价在开仓价下方` };
    } else {
      const allAbove = recent.every(k => k.close > pos.entryPrice);
      if (allAbove) return { action: 'CLOSE', reason: `连续${n}根K线收盘价在开仓价上方` };
    }
    return { action: 'HOLD' };
  }
  ...
  async checkReplenish(klines, pos) {
    ...
    // v2: 补仓前浮亏检查 — 浮亏≥10%含杠杆时不补仓
    if (this.checkReplenishLossGuard(pos)) return { action: 'HOLD', reason: `浮亏≥${CONFIG.noReplenishLossPct}%不补仓` };
  ...
  async _scan() {
    ...
    for (const symbol of Object.keys(this.positions)) {
      ...
      try {
        ...
        // 6. 补仓后均价止损
        const apStop = this.checkAvgPriceStop(pos);
        if (apStop.action === 'CLOSE') { this._log(`🔴 ${symbol} ${apStop.reason}`); await this._closePosition(symbol, pos, apStop.reason); continue; }
