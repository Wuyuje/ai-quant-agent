...
class BBScalpEngine {
  ...
  // 核心循环: 扫描固定币池, 算信号, 买卖
  async _cycle() {
    // 布林币池: 高波动且回测能盈利的币(1分钟级布林%B穿越信号多, 高抛低吸效果好)
    const defaultPool = ['HFTUSDT','VICUSDT','BICOUSDT','BLESSUSDT','1000RATSUSDT','COTIUSDT','PTBUSDT','1000PEPEUSDT','1000BONKUSDT','TURBOUSDT','NEIROUSDT','SPELLUSDT','FLOKIUSDT','PENGUUSDT','KOMAUSDT','SKYAIUSDT'];
    const pool = (this.coinPool || defaultPool).slice(0, this.maxPositions);
  ...
  async _openPos(symbol, side, price, reason) {
    ...
    if (this.realTrading) {
      try {
        const pm = await this.api.getExchangeInfo().catch(()=>null);   // precisionMap
        let r = side==='LONG' ? await this.api.marketLong(symbol, qty, lev, pm) : await this.api.marketShort(symbol, qty, lev, pm);
  ...
  async _closePos(symbol, reason) {
    ...
    if (this.realTrading) {
      try {
        const pm = await this.api.getExchangeInfo().catch(()=>null);   // precisionMap
        if (pos.side === 'LONG') await this.api.closeLong(symbol, pos.qty, pm);
        else await this.api.closeShort(symbol, pos.qty, pm);
