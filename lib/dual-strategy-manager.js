...
class DualStrategyManager {
  ...
  async _ensureEngine(wallet, apiKey, apiSecret, gatesFeeLow) {
    // v3: 只运行管理员的引擎,普通用户共用API会导致被封IP
    if (!this._isAdmin(wallet)) return;
  ...
  getAllUsersStatus() {
    ...
    for (const wallet of Object.keys(this._bbEngines)) {
      ...
      try {
        ...
        isWhitelisted = this._isAdmin(wallet);
      } catch(e) {}
      result.push({
        wallet,
        bb: bbS, trend: trendS,
    ...
    try {
      ...
      for (const [wallet, u] of Object.entries(fileUsers)) {
        ...
        const isWhitelisted = this._isAdmin(wallet);
        result.push({
          wallet,
          bb: null, trend: null,
          balance: 0,
          ...
          totalPnlUsd: 0,
          running: false,
          gatesFeePaused: !isWhitelisted && (u.gatesFeeLow || false),
