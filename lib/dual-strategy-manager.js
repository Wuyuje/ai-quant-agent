...
class DualStrategyManager {
  ...
  async _ensureEngine(wallet, apiKey, apiSecret, gatesFeeLow) {
    ...
    // v3: 跳过API Key无效的用户(解密后不是64位纯字母数字)
  ...
  getAllUsersStatus() {
    ...
    for (const wallet of Object.keys(this._bbEngines)) {
      ...
      result.push({
        ...
        gatesFeePaused: this._bbEngines[wallet]?.gatesFeePaused || false,
      });
    }
    return result;
  }
  ...
  getStats() {
    ...
    for (const bb of Object.values(this._bbEngines)) {
      ...
      bbPnl += s.totalPnlUsd;
      totalWins += bb._wins || 0;
      totalLosses += bb._losses || 0;
      totalTrades += bb._trades || 0;
      realizedPnl += bb._realizedPnl || 0;
    }
    for (const trend of Object.values(this._trendEngines)) {
      ...
      totalWins += trend._wins || 0;
      totalLosses += trend._losses || 0;
      totalTrades += trend._trades || 0;
      realizedPnl += trend._realizedPnl || 0;
    }
