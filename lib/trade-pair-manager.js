...
// 动态选币+扩充交易对: 从全市场按"当前MA7趋势质量"(非横盘+方向动量强)选TopN, 避免历史回报率选反
async function refreshPairs(opts) {
  ...
  const rows = [];
  for (const sym of pool.slice(0, 120)) {   // 限制扫描量防超时
    try {
      const score = await trendQualityScore(sym);
      if (score && score.ok) rows.push({ symbol: sym, score: score.q });
    } catch(e) {}
  }
  ...
  if (changed) updatePairs(newPairs, '全市场MA7趋势质量选币: 按当前趋势动量排行, 取前' + newPairs.length + '只');
  return { pairs: newPairs, previous: curPairs, changed, ranked: best.map(r => ({ symbol: r.symbol, score: +r.score.toFixed(2) })) };
}
