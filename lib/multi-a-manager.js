  ...
  async _ensureEngine(u) {
    ...
    if (!this._aEngines[walletKey]) {
      ...
      const engine = new AStrategySim(u.apiKey, u.apiSecret, {
        ...
        pauseOpen: this.pauseOpen || false,  // v16: 只平不开模式(停止开新仓)
      ...
      if (this._cycleCount <= 5) this._log(`✅ ${u.wallet.slice(0,10)}... A引擎启动(延迟${delay}ms)`);
    } else {
      this._aEngines[walletKey].gatesFeePaused = !!u.gatesFeeLow;
      // v16: 强制设置只平不开模式(确保已有引擎也停止开新仓)
      if (this.pauseOpen) this._aEngines[walletKey]._pauseOpen = true;
    }
  }
