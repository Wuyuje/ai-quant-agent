...
class MultiAStrategyManager {
  ...
  async _ensureEngine(u) {
    ...
    if (!this._aEngines[walletKey]) {
      ...
      this._aEngines[walletKey]._pauseOpen = true;  // 所有用户(含管理员)停开仓
      if (this._cycleCount <= 5) this._log(`${u.wallet.slice(0,10)}... ✅MA7策略暂停开仓(全停)`);
    }
  }
