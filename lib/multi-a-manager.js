...
class MultiAStrategyManager {
  constructor(opts = {}) {
    ...
    // ⛔ 暂停所有开仓(用户要求) — 可通过接口/指令改为 false 恢复
    this.pauseOpen = false;   // 放开开仓(引擎2严格版)
  ...
  // ═══ 引擎1·选币引擎: 全市场大道多指标+含费回测精选Top8交易对 ═══
  _startPairRefresh() {
    if (this._pairTimer) clearInterval(this._pairTimer);
    const run = async () => {
      try {
        const { runSelect } = require('./select-engine');
        const res = await Promise.race([
          runSelect(8),
          new Promise((_,rej)=>setTimeout(()=>rej(new Error('选币超时')), 120000))
        ]);
        if (res && res.pairs && res.pairs.length) {
          this._log(`🔁 引擎1选币更新: Top${res.pairs.length} = ${res.pairs.map(x=>x.replace('USDT','')).join(',')}`);
        }
      } catch(e) {
        this._log(`⚠️ 引擎1选币刷新失败: ${e.message.slice(0,40)}`);
      }
    };
    // 启动后60秒先跑一次, 之后每60分钟大道选币一次(全市场扫描较重)
    this._pairTimer = setInterval(run, 60 * 60 * 1000);
  ...
  async _ensureEngine(u) {
    ...
    const pauseOpenForThis = this.pauseOpen; // 遵循全局暂停开关
    if (!this._aEngines[walletKey]) {
      ...
      // 遵循全局暂停开关(this.pauseOpen), 不再硬编码false覆盖暂停
      this._aEngines[walletKey]._pauseOpen = this.pauseOpen;
      if (this._cycleCount <= 5) this._log(`${u.wallet.slice(0,10)}... ✅MA7策略${this.pauseOpen?'暂停开仓':'可开仓'}`);
    }
  }
