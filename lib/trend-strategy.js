...
const CONFIG = {
  ...
  hedgeTriggerPct: 12,      // v3: 浮亏≥12%含杠杆(价格跌4%)触发对冲
  hedgeRatio: 0.5,          // 对冲仓位=原仓位50%
  hedgeClosePct: 20,        // 原仓位浮亏≥20%时全平+平对冲仓
  ...
  async _scan(bbPositions) {
    ...
    for (const symbol of Object.keys(this.positions)) {
      ...
      try {
        ...
        // v3: 浮亏≥12%触发反向对冲
        if (!pos._hedged && tlLoss >= CONFIG.hedgeTriggerPct && tlLoss < CONFIG.hedgeClosePct) {
          try {
            ...
            this._log(`🛡️ ${symbol} 趋势对冲开仓 ${hedgeDir} qty=${hedgeQty.toFixed(4)} (原${pos.side}浮亏${tlLoss.toFixed(1)}%)`);
          } catch(e) { this._log(`⚠️ ${symbol} 对冲开仓失败: ${e.message}`); }
        }
        
        // v3: 浮亏≥20%全平+平对冲仓
        if (tlLoss >= CONFIG.hedgeClosePct) {
          if (pos._hedged) {
            try {
          ...
          this._log(`🔴 ${symbol} 总浮亏止损: ${tlLoss.toFixed(1)}%≥${CONFIG.hedgeClosePct}% (含对冲平仓)`);
          await this._closePosition(symbol, pos, `总浮亏止损: ${tlLoss.toFixed(1)}%≥${CONFIG.hedgeClosePct}%`);
          continue;
        }
        } // end grace period
