...
const CONFIG = {
  ...
  trailDrawdownPct: 2.0,     // v3: 峰值回撤2%锁利(给趋势更多空间)
...
class TrendStrategy {
  ...
  async _openPosition(symbol, direction, klines) {
    ...
    // v3: 检查总仓位(BB+趋势合计不超过10)
    const myCount = Object.keys(this.positions).length;
    const bbCount = Object.keys(this._bbPositions || {}).length;
    const totalCount = myCount + bbCount;
    const myMax = this.getDynamicMaxPositions();
    if (myCount >= myMax || totalCount >= 10) return;
  ...
  async _scan(bbPositions) {
    ...
    const bbCount2 = Object.keys(bbPositions || {}).length;
    if (myCount2 >= this.getDynamicMaxPositions() || (myCount2 + bbCount2) >= 10) return;
