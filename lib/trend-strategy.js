...
const CONFIG = {
  ...
  // ═══ 增强风控配置(修复缺失,接入止损) ═══
  atrStopMultiplier: 12,        // 趋势仓允许12个ATR反向偏移(更宽,给趋势空间)
  singleKLossPct: 6,            // 单K浮亏≥6%(不含杠杆)止损
  ultimateLossPct: 15,          // 终极止损(不含杠杆)
  consecutiveLossPct: 2.5,      // 连续K线偏离开仓价≥2.5%(不含杠杆)
  consecutiveLossKlines: 10,    // 连续10根K线反方向
  noReplenishLossPct: 10,       // 浮亏≥10%不含杠杆时不补仓
  liquidationWarnPct: 0.80,     // 强平预警80%
  maxHoldHours: 4,              // 时间止损阈值
  maxHoldLossPct: 5,            // >4h且浮亏≥5%含杠杆
  avgPriceLossPct: 8,           // 补仓后价格偏离均价≥8%(不含杠杆)
...
class TrendStrategy {
  ...
  async _openPosition(symbol, direction, klines) {
    ...
    let margin = Math.min(this.balance * this._getPositionPct(atrPct), 30); // v3: 单仓上限$30
    ...
    if (this.accountGuard) {
      ...
      if (!cap.allowed) { this._log(`⏭️ ${symbol} 趋势: 账户容量不足,不开仓: ${cap.reason}`); return; }
      if (cap.limitNotional < notional) {
        margin = Math.max(0, cap.limitNotional / CONFIG.leverage);
        if (margin < 1) { this._log(`⏭️ ${symbol} 趋势: 账户剩余容量过小,不开仓`); return; }
      }
    }
  ...
  async _scan(bbPositions) {
    ...
    for (const symbol of Object.keys(this.positions)) {
      ...
      try {
        ...
        if (tlLoss >= CONFIG.reverseTriggerPct) {
          this._log(`🔴 ${symbol} 浮亏止损: ${tlLoss.toFixed(1)}%≥${CONFIG.reverseTriggerPct}%`);
        ...
        // 单K止损
        const singleSl = this.checkSingleKStopLoss(klines, pos);
        if (singleSl.action === 'CLOSE') { this._log(`🔴 ${symbol} ${singleSl.reason}`); await this._closePosition(symbol, pos, singleSl.reason); continue; }
        // 终极止损
