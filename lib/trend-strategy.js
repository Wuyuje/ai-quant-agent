  ...
  async _scan(bbPositions) {
    ...
    for (const symbol of Object.keys(this.positions)) {
      ...
      try {
        ...
        // v3: 浮亏≥10%减仓50%+开反向新仓
        if (!pos._reduced && tlLoss >= CONFIG.reduceLossPct && tlLoss < CONFIG.fullStopPct) {
          try {
            ...
            // 用释放的保证金开反向新仓(仓位不满时)
            let binanceCount = Object.keys(this.positions).length;
            try {
              const remote = await Promise.race([this.api.getPositions(), new Promise((_,rej) => setTimeout(() => rej(new Error('timeout')), 8000))]).catch(e => null);
              if (remote) binanceCount = remote.filter(p => Math.abs(parseFloat(p.positionAmt)) > 0).length;
            } catch(e) {}
        ...
        // v3: 浮亏≥20%原仓全平(反向仓继续持有)
        if (tlLoss >= CONFIG.fullStopPct) {
          this._log(`🔴 ${symbol} 总浮亏止损: ${tlLoss.toFixed(1)}%≥${CONFIG.fullStopPct}% 原仓全平,反向仓继续`);
          await this._closePosition(symbol, pos, `总浮亏止损: ${tlLoss.toFixed(1)}%≥${CONFIG.fullStopPct}%`);
          continue;
        }
        } // end grace period
