...
class AStrategySim {
  ...
  async checkSignal(klines, symbol) {
    ...
    // 做多: 底部区 + MA7最新明确向上(放宽prevSeg限制, 只要拐头方向对)
    const lowBuy = atBottom && latest > turnMinAbs;
    // 做空: 顶部区 + MA7最新明确向下(放宽prevSeg限制)
    const highSell = atTop && latest < -turnMinAbs;
    if (lowBuy) {
      return { allowed:true, direction:'LONG', reason:`长期底部低买(位置${(posRatio*100).toFixed(0)}%底部 区间${spreadPct.toFixed(1)}% 拐上)`, confidence:0.78, atrPct:0.4 };
    }
    if (highSell) {
      return { allowed:true, direction:'SHORT', reason:`长期顶部高卖(位置${(posRatio*100).toFixed(0)}%顶部 区间${spreadPct.toFixed(1)}% 拐下)`, confidence:0.78, atrPct:0.4 };
    }
  ...
  async checkTakeProfit(pos, klines) {
    // 高位/低位平仓 — 要求MA7从底位运行到高位(趋势走了一段) + 反转拐头才平, 避免开仓即平
    // 平多: MA7从持仓低位涨到高位区 + 最新拐头向下 → 高位平仓(趋势到顶反转)
    // 平空: MA7从持仓高位跌到低位区 + 最新拐头向上 → 低位平仓(趋势到底反转)
    ...
    const n = ma.length;
    const curMA = ma[n-1];
    // 持仓以来MA7最低/最高(判断MA7是否已从底位走到高位)
    const entryMA = pos._entryMA || curMA;   // 开仓时MA7
    if (!pos._entryMA) pos._entryMA = entryMA;
    const hist = ma.slice(-40);
    const mx = Math.max(...hist), mn = Math.min(...hist);
    const range = (mx - mn) || 1;
    const posRatio = (curMA - mn) / range;   // 当前MA7位置
    const m4 = ma[n-2], m5 = ma[n-1];        // 最新拐头
    const pnlPct = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnlPct;
    const highCut = CONFIG.takeProfitHighCut || 0.6;   // 高位区
    const lowCut  = CONFIG.takeProfitLowCut || 0.4;    // 低位区
    // ═══ 逆势平仓保护: 持仓方向与MA7趋势明显相反(逆势)即平, 防逆势扛单 ═══
    // 做多却MA7大幅低于开仓(跌势,逆多) → 平; 做空却MA7大幅高于开仓(涨势,逆空) → 平
    const gotFallbackPct = CONFIG.reverseClosePct || 4;   // MA7相对开仓反向跑≥4%(5分钟级别)视为逆势
    if (pos.side === 'LONG') {
      const fellPct = (pos._entryMA - curMA) / (pos._entryMA||1) * 100;
      if (pos._entryMA && fellPct >= gotFallbackPct) {
        return { action:'CLOSE', reason:`逆势平多保护(做多但MA7从开仓${pos._entryMA.toFixed(4)}跌${fellPct.toFixed(1)}%≥${gotFallbackPct}%,防逆势扛单,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
      }
    } else if (pos.side === 'SHORT') {
      const rosePct = (curMA - pos._entryMA) / (pos._entryMA||1) * 100;
      if (pos._entryMA && rosePct >= gotFallbackPct) {
        return { action:'CLOSE', reason:`逆势平空保护(做空但MA7从开仓${pos._entryMA.toFixed(4)}涨${rosePct.toFixed(1)}%≥${gotFallbackPct}%,防逆势扛单,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
      }
    }
    if (pos.side === 'LONG') {
      // 平多: MA7在高位区 + 拐头向下 + 已从开仓位置涨上来(不是开仓就在高位套牢)
      const roseFromEntry = curMA > pos._entryMA;   // MA7高于开仓时(确实从底位涨上来)
      if (posRatio > highCut && m5 < m4 && roseFromEntry) {
        return { action:'CLOSE', reason:`高位平仓平多(MA7位${(posRatio*100).toFixed(0)}%从开仓${pos._entryMA.toFixed(4)}涨到${curMA.toFixed(4)}后拐头向下,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
      }
    } else if (pos.side === 'SHORT') {
      // 平空: MA7在低位区 + 拐头向上 + 已从开仓位置跌下来(不是开仓就在低位)
      const fellFromEntry = curMA < pos._entryMA;
      if (posRatio < lowCut && m5 > m4 && fellFromEntry) {
        return { action:'CLOSE', reason:`低位平仓平空(MA7位${(posRatio*100).toFixed(0)}%从开仓${pos._entryMA.toFixed(4)}跌到${curMA.toFixed(4)}后拐头向上,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
      }
    }
    return { action: 'HOLD' };
  }
