...
class AStrategySim {
  ...
  async checkSignal(klines, symbol) {
    // (大道至简v10·趋势版) 5分钟MA7顺势拐头信号 — 方向由主循环的15m趋势过滤决定
    // 这里只检测: MA7由下行转上行(上拐) 报 LONG; 由上行转下行(下拐) 报 SHORT
    // 真正的"做多/做空方向选择"由 _getSymbolTrend(15m EMA20/60+ADX) 决定: 顺大势才按此信号开仓
    if (!klines || klines.length < 40) return { allowed: false, reason: 'K线不足' };
    ...
    const turnMin = CONFIG.maTurnThreshold || 0.0005;  // 最小拐头幅度(过滤微抖)
    // 上拐: 前一根下/平 → 最新转上
    if (latest > turnMin && prevSeg <= 0) {
      return { allowed:true, direction:'LONG', reason:`MA7上拐(转升${(latest*100).toFixed(2)}%)` , confidence:0.7, atrPct:0.4 };
    }
    // 下拐: 前一根上/平 → 最新转下
    if (latest < -turnMin && prevSeg >= 0) {
      return { allowed:true, direction:'SHORT', reason:`MA7下拐(转降${(latest*100).toFixed(2)}%)`, confidence:0.7, atrPct:0.4 };
    }
    return { allowed:false, reason:`MA7拐头不足(最新${(latest*100).toFixed(3)}%)` };
  }
  ...
  async _scan() {
    ...
    for (const symbol of symbols) {
      ...
      try {
        ...
        if (signal.allowed) {
          // ★★ 趋势交易核心: 开仓必须顺15m趋势 + 趋势强度够(ADX≥18非横盘)
          // 只有趋势明确时才顺势开仓, 逆势/横盘一律不开 → 避免震荡频繁反向亏
          let trendOk = true;
          const tr = await this._getSymbolTrend(symbol);
          if (CONFIG.requireSymbolTrend !== false) {
            // 趋势方向须与开仓方向一致(顺趋势)且趋势有强度(ADX≥18)
            if (signal.direction === 'LONG' && !(tr.dir === 'UP' && tr.strong)) trendOk = false;
            else if (signal.direction === 'SHORT' && !(tr.dir === 'DOWN' && tr.strong)) trendOk = false;
            // 横盘/趋势不明也不开
            if (!trendOk) {
              this._log(`◀ ${symbol} ${signal.direction}信号但${tr.dir==='FLAT'?'横盘':('逆15m趋势'+tr.dir+'/ADX'+tr.adx)},不开仓(顺趋势)`, tr);
              continue;
            }
          }
          signal._price = klines[klines.length - 1].close;
          signal._klines = klines;
          this._log(`🟢 ${symbol} ${signal.direction}MA7信号(顺15m${tr.dir}/ADX${tr.adx}): ${signal.reason}`);
