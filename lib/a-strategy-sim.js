...
const CONFIG = {
  ...
  // 只交易这3个币
  tradeSymbols: ["BTCUSDT","BNBUSDT","BLESSUSDT","HOMEUSDT","PTBUSDT","1000SATSUSDT","NEIROUSDT"],
  // MA7拐头策略
  maPeriod: 7,                  // MA7均线
...
class AStrategySim {
  ...
  // 只做固定3只币(BTC/BNB/ZEC)
  async selectSymbols() {
    return [...(CONFIG.tradeSymbols || ['BTCUSDT','BNBUSDT','ZECUSDT'])];
  }
  ...
  async checkSignal(klines, symbol) {
    if (!klines || klines.length < 30) return { allowed: false, reason: 'K线不足' };
    const closes = klines.map(k => k.close);
    // MA7序列
    const ma = [];
    for (let i = Math.max(0, closes.length-20); i <= closes.length-7; i++) {
      const v = Indicators.sma(closes.slice(i, i+7), 7);
      if (v != null) ma.push(v);
    }
    if (ma.length < 3) return { allowed:false, reason:'MA不足' };
    const n = ma.length;
    // 前段方向: 看 MA7 前几根(ma[n-4]~ma[n-2])的趋势
    const a1=ma[n-4], a2=ma[n-3], a3=ma[n-2], cur=ma[n-1], prev=ma[n-2];
    const flat = CONFIG.maFlatThreshold || 0.00035;
    // 前段累计方向(用 a1→a3): 若 a3 < a1*(1-0.001) = 前段下行
    const prevUpDown = (a3 - a1) / a1;         // 前段MA7总方向: >0上行,<0下行
    const curChg = (cur - prev) / prev;        // 当期变化
    const prevChg = (prev - a3) / a3;          // 上一期变化
    // 前段必须有明确方向(累计|prevUpDown| ≥ 2*flat),否则是横盘
    const clearPrior = Math.abs(prevUpDown) >= 2*flat && Math.abs(prevUpDown) >= Math.abs(curChg)*0.5;
    if (!clearPrior) return { allowed:false, reason:`横盘/方向未立(前段${(prevUpDown*10000).toFixed(1)}万分比)不开仓` };
    // 做多夹角: 前段下行(prevUpDown<0) + 当前急转向上(curChg>0且明显, prevChg<0)
    if (prevUpDown < 0 && curChg > flat*2 && prevChg < 0 && cur > prev) {
      // 且夹角明显: 当期涨幅 >= 前段跌幅的一半(真V形,非微小上翘)
      if (Math.abs(curChg) >= Math.abs(prevUpDown)*0.5) {
        return { allowed:true, direction:'LONG', reason:`MA7拐头夹角做多(前段${(prevUpDown*100).toFixed(3)}%后急转+${(curChg*100).toFixed(3)}%)`, confidence:0.7, atrPct:0.4 };
      }
    }
    // 做空夹角: 前段上行(prevUpDown>0) + 当前急转向下(curChg<0明显)
    if (prevUpDown > 0 && curChg < -flat*2 && prevChg > 0 && cur < prev) {
      if (Math.abs(curChg) >= Math.abs(prevUpDown)*0.5) {
        return { allowed:true, direction:'SHORT', reason:`MA7拐头夹角做空(前段+${(prevUpDown*100).toFixed(3)}%后急转${(curChg*100).toFixed(3)}%)`, confidence:0.7, atrPct:0.4 };
      }
    }
    return { allowed:false, reason:`MA7无夹角信号(前段${(prevUpDown*100).toFixed(3)}%当期${(curChg*100).toFixed(3)}%)` };
  }
  ...
  // (重写) 止盈: 三线趋势止盈
  // (大道至简) 止盈: MA7拐头反向即平仓
  // 止盈: MA7形成反向夹角才平仓(防微变误平)
  checkTakeProfit(pos, klines) {
    if (!klines || klines.length < 20) return { action: 'HOLD' };
    const closes = klines.map(k => k.close);
    const ma = [];
    for (let i=Math.max(0,closes.length-12); i<=closes.length-7; i++){ const v=Indicators.sma(closes.slice(i,i+7),7); if(v!=null) ma.push(v); }
    if(ma.length<4) return {action:'HOLD'};
    const n=ma.length, a2=ma[n-4], a1=ma[n-3], prev=ma[n-2], cur=ma[n-1];
    const flat=CONFIG.maFlatThreshold||0.00035;
    const pnlPct=this._calcPnlPct(pos,pos.currentPrice);
    pos._lastPnlPct=pnlPct;
    const prior=(a1-a2)/a2;      // 前段方向
    const chg=(cur-prev)/prev;   // 当期变化
    const prevChg=(prev-a1)/a1;
    if (pos.side==='LONG') {
      // 做多止盈: 前段上行 + 急转向下夹角
      if (prior>0 && chg<-flat*2 && prevChg>0 && cur<prev && Math.abs(chg)>=Math.abs(prior)*0.5) {
        return {action:'CLOSE', reason:`做多夹角止盈(前段+${(prior*100).toFixed(2)}%后急转${(chg*100).toFixed(2)}%) ${pnlPct>=0?'落袋':'止损'}`};
      }
    } else if (pos.side==='SHORT') {
      // 做空平仓: 前段下行 + 急转向上夹角
      if (prior<0 && chg>flat*2 && prevChg<0 && cur>prev && chg>=Math.abs(prior)*0.5) {
        return {action:'CLOSE', reason:`做空夹角平仓(前段${(prior*100).toFixed(2)}%后急转+${(chg*100).toFixed(2)}%) ${pnlPct>=0?'落袋':'止损'}`};
      }
    }
    return {action:'HOLD'};
  }
  ...
  checkStopLoss(pos, klines) {
    const pnlPct=this._calcPnlPct(pos,pos.currentPrice);
    pos._lastPnlPct=pnlPct;
    // 硬止损兜底: 浮亏≥15%(高杠杆长线)强制止损
    if (pnlPct <= -15) return { action:'CLOSE', reason:`止损: 浮亏${pnlPct.toFixed(1)}%≥15%` };
  ...
  async _syncPositions() {
    ...
    try {
      ...
      const allowedSet = new Set(CONFIG.tradeSymbols || ['BTCUSDT','BNBUSDT','ZECUSDT']);
