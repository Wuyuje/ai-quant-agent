...
const CONFIG = {
  ...
  maxPairs: 10,               // 全市场选Top10只最佳币进交易对(MA7拐头+回报率/胜率优选)
  // v18 长期底/顶判定
  ...
  consecK: 3,                 // 开仓要求MA7连续3根同向=趋势形成才开(平仓则单根拐头即平)
...
class AStrategySim {
  ...
  async selectSymbols() {
    ...
    try {
      ...
      if (dyn && Array.isArray(dyn) && dyn.length > 0) return [...dyn].slice(0, CONFIG.maxPairs || 10);
    } catch(e) {}
    ...
    try {
      ...
      if (allCoins && allCoins.length) return [...allCoins].slice(0, CONFIG.maxPairs || 10);
    } catch(e) {}
    return [...(CONFIG.tradeSymbols || [])].slice(0, CONFIG.maxPairs || 10);
  }
  ...
  async checkSignal(klines, symbol) {
    // (大道至简v18·长期底/顶判定) — MA7在行情长期实际底部"+上拐 = 做多(低买); 长期顶部"+下拐 = 做空(高卖)
    // 位置不固定看N根: 用本段行情的实际高低区间(最多200根)判当前MA7位置
    // 横盘(区间太窄)不做; 只在趋势确定性高的币做
    ...
    const n = ma.length;
    const curMA = ma[n-1];
    // ── 长周期位置判定: 用近200根MA7(或全部)确定本段行情的底部/顶部 ──
    const lookback = Math.min(200, n);          // 最多看近200根MA7(多根才准)
    const hist = ma.slice(-lookback);
    const maxMA = Math.max(...hist), minMA = Math.min(...hist);
    const MA_range = maxMA - minMA;
    // ── 横盘/无趋势过滤: 区间太窄(接近0)说明横盘/波动不够, 不做 ──
    const volBase = Math.abs(maxMA) || 1;
    const spreadPct = MA_range / volBase * 100;   // 区间相对幅度%
    const minSpread = CONFIG.minSpreadPct || 2;   // 区间需≥2%才有趋势(放宽)
    if (MA_range <= 0 || spreadPct < minSpread) {
      return { allowed:false, reason:`横盘跳过: 区间幅度${spreadPct.toFixed(1)}%<${minSpread}%(无趋势)` };
    }
    // 当前MA7在区间的位置(0=最底, 1=最顶)
    const posRatio = (curMA - minMA) / MA_range;
    // 拐头信号
    const latest = (ma[n-1] - ma[n-2]);
    const prevSeg = (ma[n-2] - ma[n-3]);
    const turnMinAbs = (MA_range * (CONFIG.maTurnRatio || 0.008)); // 拐头幅度=区间幅度的2.5%(放宽,易触发)
    const baseX = Math.abs(curMA) || 1;
    // ── 只在极底部或极顶部 + 明确拐头才开 ──
    const bottomCut = CONFIG.bottomCut || 0.28;   // 位置<28% = 底部区(放宽)
    const topCut    = CONFIG.topCut || 0.72;      // 位置>72% = 顶部区(放宽)
    const atBottom = posRatio < bottomCut;
    const atTop    = posRatio > topCut;
    // ═══ 开仓: 连续几根同向(趋势形成确认) + 底/顶部位置 ═══
    const consecK = CONFIG.consecK || 3;           // 要求连续3根MA7同向 = 趋势形成
    const conj = ma.slice(n - consecK, n);         // 最近consecK根MA7
    const upStreak = conj.every((v,i,a)=> i===0 ? true : (v - a[i-1]) > 0);   // 连续上升(趋势形成)
    const dnStreak = conj.every((v,i,a)=> i===0 ? true : (v - a[i-1]) < 0);   // 连续下降(趋势形成)
    // 做多: 底部区 + MA7连续consecK根向上(趋势形成确认) + 幅度足
    const lowBuy = atBottom && upStreak && latest > turnMinAbs;
    // 做空: 顶部区 + MA7连续consecK根向下(趋势形成确认) + 幅度足
    const highSell = atTop && dnStreak && latest < -turnMinAbs;
    if (lowBuy) {
      return { allowed:true, direction:'LONG', reason:`底部低买(位${(posRatio*100).toFixed(0)}% +MA7连${consecK}根向上趋势形成)`, confidence:0.78, atrPct:0.4 };
    }
    if (highSell) {
      return { allowed:true, direction:'SHORT', reason:`顶部高卖(位${(posRatio*100).toFixed(0)}% +MA7连${consecK}根向下趋势形成)`, confidence:0.78, atrPct:0.4 };
    }
    return { allowed:false, reason:`位${(posRatio*100).toFixed(0)}%(需<${bottomCut*100}%或多于${topCut*100}%) 区间${spreadPct.toFixed(1)}%` };
  }

  
  async checkTakeProfit(pos, klines) {
    // 高位/低位平仓判断(修正) — 平仓须"真的走到高位/低位"(相对开仓有明显涨跌)才平, 不是看百分比位置
    // 平多(到高位): 做多真涨了明显一段(相对开仓涨幅≥riseToClose) + MA7连续向下(明显反转) → 平多
    // 平空(到底位): 做空真跌了明显一段(相对开仓跌幅≥riseToClose) + MA7连续向上(明显反转) → 平空
    // 修正: 不再用"MA7位置百分比"判断高位(会开仓时就在高位误平), 改用相对开仓的实际涨跌幅度
    ...
    // 明显反转: MA7连续consecK根反向(与持仓方向相反) — 平仓也要确认反转,不是单根拐头
    const consecK = CONFIG.consecK || 3;   // 与开仓一致,要求连续3根反向才视为明显反转
    const recentMA = ma.slice(n - consecK, n);   // 最近consecK根MA7
    const dnRev = recentMA.every((v,i,a)=> i===0 ? true : (v - a[i-1]) <= 0);   // 连续3根向下(明显反转,平多用)
    const upRev  = recentMA.every((v,i,a)=> i===0 ? true : (v - a[i-1]) >= 0);  // 连续3根向上(明显反转,平空用)
    const pnlPct = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnlPct;
    // 相对开仓MA7的涨跌幅%(判定是否真到高位/低位)
    const movePct = (curMA - pos._entryMA) / (pos._entryMA||1) * 100;
    // 到高位/低位所需涨幅(5分钟级别, 相对开仓明显走了一段)
    const riseToClose = CONFIG.riseToClosePct || 1.5;   // 涨/跌≥1.5%才算真到高位/低位
    // ═══ 逆势平仓保护(优先): 持仓方向与MA7趋势明显相反(反向跑≥4%)即平 ═══
    const gotFallbackPct = CONFIG.reverseClosePct || 4;
    if (pos.side === 'LONG') {
      if (pos._entryMA && movePct <= -gotFallbackPct) {
        return { action:'CLOSE', reason:`逆势平多(做多但MA7跌${(-movePct).toFixed(1)}%≥${gotFallbackPct}%,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
      }
    } else if (pos.side === 'SHORT') {
      if (pos._entryMA && movePct >= gotFallbackPct) {
        return { action:'CLOSE', reason:`逆势平空(做空但MA7涨${movePct.toFixed(1)}%≥${gotFallbackPct}%,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
      }
    }
    // 正常止盈: 真涨到高位(做多) / 真跌到底位(做空) + 出现拐头
    if (pos.side === 'LONG') {
      // 做多: 相对开仓涨≥riseToClose(真到高位) + MA7最新拐头向下 → 高位平多
      if (pos._entryMA && movePct >= riseToClose && dnRev) {
        return { action:'CLOSE', reason:`高位平多(MA7从开仓${pos._entryMA.toFixed(4)}涨${movePct.toFixed(1)}%+拐头向下,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
      }
    } else if (pos.side === 'SHORT') {
      // 做空: 相对开仓跌≥riseToClose(真到底位) + MA7最新拐头向上 → 低位平空
      if (pos._entryMA && movePct <= -riseToClose && upRev) {
        return { action:'CLOSE', reason:`低位平空(MA7从开仓${pos._entryMA.toFixed(4)}跌${(-movePct).toFixed(1)}%+拐头向上,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
      }
    }
    return { action: 'HOLD' };
  }
