...
const CONFIG = {
  ...
  profitTriggerPct: 6.0,      // v9: 浮盈≥6%才触发移动止盈(让盈利单多跑,提高盈亏比)
  trailDrawdownPct: 3.0,      // v9: 回撤3%才锁利(不轻易止盈,给趋势空间)
  totalLossPct: 8,            // v9: 硬止损8%(单笔亏损受控) — 盈亏比6%vs8%
  softLossPct: 5,             // v9: 软止损5%
...
class AStrategySim {
  ...
  checkStopLoss(pos, klines) {
    ...
    // 软止损+趋势确认: 空单浮亏≥4.5%,多单浮亏≥5%,且趋势对持仓不利 → 先减仓一半(分批止损防假止损)
    const softLoss = pos.side === 'SHORT' ? Math.max(4.5, CONFIG.softLossPct - 0.5) : CONFIG.softLossPct;
