...
class AStrategySim {
  ...
  async checkSignal(klines, symbol) {
    ...
    // 做多: EMA多头 + RSI<50 + MACD柱增长
    if (isUptrend && rsi < 50 && macd.hist > macd.prevHist) {
      direction = 1;
      indicatorReason = `EMA多头+RSI=${rsi.toFixed(0)}+MACD增长`;
    }
    // 做空: EMA空头 + RSI>50 + MACD柱减少
    else if (isDowntrend && rsi > 50 && macd.hist < macd.prevHist) {
      direction = -1;
      indicatorReason = `EMA空头+RSI=${rsi.toFixed(0)}+MACD减少`;
    }
    ...
    if (nnPred) {
      ...
      if (nnDir === direction) aiScore += 0.2; // AI方向一致加分
    }
    ...
    for (const symbol of symbols) {
      ...
      try {
        ...
        if (signal.allowed) {
          ...
          signal._klines = klines; // v3: 保存K线用于训练
