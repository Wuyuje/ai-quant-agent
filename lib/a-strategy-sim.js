...
class AStrategySim {
  ...
  async checkSignal(klines, symbol) {
    if (!klines || klines.length < 20) return { allowed: false, reason: 'K线不足' };
    const closes = klines.map(k => k.close);
    // 算 MA7 序列(最近7个点)
    const ma = [];
    for (let i = Math.max(0, closes.length-13); i <= closes.length-7; i++) {
      const v = Indicators.sma(closes.slice(i, i+7), 7);
      if (v != null) ma.push(v);
    }
    if (ma.length < 5) return { allowed:false, reason:'MA不足' };
    const n = ma.length;
    const m1=ma[n-5], m2=ma[n-4], m3=ma[n-3], m4=ma[n-2], m5=ma[n-1];
    // 前段方向: m1→m3 是下行还是上行
    const priorDown = m3 < m1;   // 前段在走下行
    const priorUp   = m3 > m1;   // 前段在走上行
    // 拐头强度: 看最后两点(m4→m5)是否急转
    const turn = (m5 - m4) / m4; // 当期MA7变化
    const flat = CONFIG.maFlatThreshold || 0.00035;
    // 前段下行 → 突然掉头向上(且涨幅足够形成夹角)
    if (priorDown && turn > flat*3 && m5 > m4 && m4 < m3) {
      // m4仍低于m3确认下行末端, m5急升 = 夹角底部
      return { allowed:true, direction:'LONG', reason:`MA7向上夹角(底部急升${(turn*100).toFixed(3)}%)`, confidence:0.7, atrPct:0.4 };
    }
    // 前段上行 → 突然掉头向下
    if (priorUp && turn < -flat*3 && m5 < m4 && m4 > m3) {
      return { allowed:true, direction:'SHORT', reason:`MA7向下夹角(顶部急跌${(turn*100).toFixed(3)}%)`, confidence:0.7, atrPct:0.4 };
    }
    return { allowed:false, reason:`MA7无夹角(当期${(turn*100).toFixed(4)}%)` };
  }
