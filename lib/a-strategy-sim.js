...
const CONFIG = {
  ...
  profitTriggerPct: 4.0,      // v10fix: 浮盈≥4%触发移动止盈(之前8%太高,盈利单没保护被反转).4%让盈利单及早保护
  trailDrawdownPct: 1.5,      // v10fix: 峰值回撤1.5%锁利(赚到4%后,回撤1.5%就锁利,落袋为安)
  breakEvenPct: 1.5,          // v10fix: 浮盈≥1.5%后触发保本保护(回落开仓价则保本平仓,防从盈变亏)
  totalLossPct: 5,            // v10fix: 硬止损5%保持
  softLossPct: 4,             // v10fix: 软止损4%
...
class AStrategySim {
  ...
  checkTakeProfit(pos) {
    ...
    // v10fix: 保本/锁利保护 — 浮盈达breakEvenPct后,若峰值回吐过半,锁住已得利润(防从盈变亏)
