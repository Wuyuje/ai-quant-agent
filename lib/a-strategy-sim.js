...
const CONFIG = {
  ...
  // ═══ 大道至简重写 v3 ═══
  // 选币/开仓: 5分钟级别 MA7 翻头(反转) + 看至少一天(288根5m)K线定位高低区
  // 做多低买: MA7在低位区(一天底部)且刚拐头向上 → 买入
  // 做空高卖: MA7在高位区(一天顶部)且刚拐头向下 → 卖出
  // 平仓止盈: 15分钟级别 MA7 高位反转向下平多 / 底位反转向上平空
  // 原则: 只做趋势, 方向绝不反, 大趋势确定后吃满整波
  maPeriod: 7,                  // MA7均线
  klineInterval: '5m',          // 开仓看5分钟级别
  klineLimit: 300,              // 5m开仓拉300根(≥一天288根, 覆盖24小时定位)
  takeProfitInterval: '15m',    // 止盈平仓看15分钟级别
  takeProfitLimit: 120,         // 15m平仓拉120根(≥一天96根, 覆盖24小时定位)
  dayLookback5m: 288,           // 5m一天=288根(定位开仓高低区)
  dayLookback15m: 96,           // 15m一天=96根(定位平仓高低区)
  // 做多: MA7急拐向上 → 逐仓高杠杆; MA7拐向下止盈
  longLeverage: 8,              // 做多8x(趋势单)
  longMarginPct: 0.10,          // 做多单次用10%资金
  // 做空: 高卖低买, 与做多对称
  shortLeverage: 8,             // 做空8x(趋势单)
...
class AStrategySim {
  ...
  // ═══ 选币: 全市场流动性池, 由checkSignal(5m MA7反转+一天定位)决定开仓 ═══
  // 大道至简: 选币用5分钟级别MA7看反转 — 拉全市场USDT永续流动性币列表,
  // 具体开哪只在 _scan 里逐个 checkSignal(5m MA7低位反转做多/高位反转做空) 过滤.
  async selectSymbols() {
    try {
      ...
      if (allCoins && Array.isArray(allCoins) && allCoins.length) {
        // 优先用动态交易对(若有), 否则用全市场流动性池, 交给checkSignal按5m MA7反转过滤
        try {
  ...
  // 大道至简: 只做趋势, 方向绝不反.
  // 做多低买: 5m看一天K线, MA7在低位区(当日底)且刚拐头向上 → 买入
  // 做空高卖: 5m看一天K线, MA7在高位区(当日顶)且刚拐头向下 → 卖出
  async checkSignal(klines, symbol) {
    ...
    // 至少看一天(5m的288根=24h)定位高低区 — 用当日MA7最高/最低
    const day = CONFIG.dayLookback5m || 288;
    const lookback = Math.min(day, n);
    ...
    if (MA_range <= 0 || spreadPct < minSpread) {
      return { allowed: false, reason: `横盘跳过(一天区间${spreadPct.toFixed(1)}%<${minSpread}%)` };
    }

    // 当前MA7在当日高低位置: 0=最低 1=最高
    ...
    const lowCut  = CONFIG.cysLowCut || 0.44;   // <44% = 当日低位区
    const highCut = CONFIG.cysHighCut || 0.56;  // >56% = 当日高位区
    ...
    if (atLow && bullishTurn && notAntiTrendLow) {
      return { allowed: true, direction: 'LONG', reason: `低买(位${(posRatio*100).toFixed(0)}%当日底,MA7拐上)`, confidence: 0.8, atrPct: 0.4 };
    }
    if (atHigh && bearishTurn) {
      return { allowed: true, direction: 'SHORT', reason: `高卖(位${(posRatio*100).toFixed(0)}%当日顶,MA7拐下)`, confidence: 0.8, atrPct: 0.4 };
    }
  ...
  async checkTakeProfit(pos, klines) {
    // ═══ 止盈平仓: 15分钟级别 K线, 看一天(96根15m)定位高低区, MA7反转变盘平仓 ═══
    // 做多: 15m看一天, MA7到高位区(当日顶)且拐头向下 → 平多(低买高卖, 大趋势到顶离场)
    // 做空: 15m看一天, MA7到底位区(当日底)且拐头向上 → 平空(高卖低买, 大趋势到底离场)
    // 原则: 只在大趋势确定后平仓(趋势到位+拐头反转), 方向绝不反
    const price = parseFloat(pos.currentPrice) || 0;
    if (!price) return { action: 'HOLD' };
    try {
      // 拉15分钟K线(止盈平仓级别)
      const tpLimit = CONFIG.takeProfitLimit || 120;
      const interval = CONFIG.takeProfitInterval || '15m';
      const tk = await Promise.race([
        this.api.getKlines(pos.symbol, interval, tpLimit),
        new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), 10000))
      ]).catch(() => null);
      if (!tk || tk.length < 60) return { action: 'HOLD' };
      ...
      if (tma.length < 60) return { action: 'HOLD' };
      const nm = tma.length;
      const curTMA = tma[nm - 1];

      // 15m看一天(96根)定位当日高低区
      const day = CONFIG.dayLookback15m || 96;
      const look = Math.min(day, nm);
      ...
      const posRatio = (curTMA - minTMA) / tRange;  // 0=当日最低 1=当日最高
      ...
      if (pos.side === 'LONG') {
        // 平多: 到当日高位区(>highCut) + MA7拐头向下(大趋势到顶反转)
        const highCut = CONFIG.takeProfitHighCut || 0.65;
        const topTurn = d1 < -turnMinAbs;
        if (posRatio > highCut && topTurn) {
          const pnl = this._calcPnlPct(pos, price);
          return { action: 'CLOSE', reason: `15m高位平多(位${(posRatio*100).toFixed(0)}%当日顶,MA7拐下,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
      } else if (pos.side === 'SHORT') {
        // 平空: 到底部区(<lowCut) + MA7拐头向上(大趋势到底反转)
        const lowCut = CONFIG.takeProfitLowCut || 0.35;
        const botTurn = d1 > turnMinAbs;
        if (posRatio < lowCut && botTurn) {
          const pnl = this._calcPnlPct(pos, price);
          return { action: 'CLOSE', reason: `15m底位平空(位${(posRatio*100).toFixed(0)}%当日底,MA7拐上,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
      }
    } catch (e) { /* 平仓计算失败则继续持有 */ }
    return { action: 'HOLD' };
  ...
  checkStopLoss(pos, klines) {
    // ═══ 防爆仓保护(高杠杆必须, 趋势交易兜底) ═══
    // 高杠杆(12x/5x)下, 浮亏达爆仓前预警线强制平仓, 防爆仓归零
    const pnl = this._calcPnlPct(pos, pos.currentPrice) || 0;
    pos._lastPnlPct = pnl;
    const lev = pos.leverage || 8;
    // 爆仓预警线: 浮亏达 ~60%保证金本金(远在爆仓前) 强制平, 保住剩余资金
    const liqProtect = CONFIG.liqProtectPct || 60;
    if (pnl <= -liqProtect) {
      return { action:'CLOSE', reason:`⚠️防爆仓保护平仓(浮亏${pnl.toFixed(1)}%≥${liqProtect}%,${lev}x杠杆,强制平防爆仓)` };
    }
    // 层级警示: 浮亏达50%, 减仓一半降风险(未到60%硬线先减半)
    if (pnl <= -50 && !pos._liqHalf) {
      pos._liqHalf = true;
      return { action:'HALF', reason:`⚠️防爆仓减半(浮亏${pnl.toFixed(1)}%≥50%,先减半降风险)` };
    }
    return { action:'HOLD' };
  }
  ...
  async _scan() {
    ...
    for (const symbol of symbols) {
      ...
      try {
        ...
        // ★ 大道至简: 5m MA7反转 + 一天K线定位 — 低位拐上做多低买, 高位拐下做空高卖
