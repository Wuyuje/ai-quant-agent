...
const CONFIG = {
  ...
  maxHoldKlines: 2016,          // 最多持仓2016根5分钟K线(7天) — 趋势单可长持, 不因超时提前离场
...
class AStrategySim {
  ...
  async checkTakeProfit(pos, klines) {
    // (大道至简v15·MA7拐头止盈) — 平仓看MA7线拐头: 趋势中突然反向拐头即平
    // 做多: MA7上行中忽然拐头向下(到顶) → 平仓止盈
    // 做空: MA7下行中忽然反转拐头向上(到底) → 平仓止盈
    // 兜底: 15m趋势反转也平(防MA7在横盘钝化)
    if (!klines || klines.length < 15) return { action:'HOLD' };
    // 1) MA7拐头平仓(主信号)
    const closes = klines.map(k => k.close);
    const ma = [];
    for (let i = Math.max(0, closes.length-14); i <= closes.length-7; i++) {
      const v = Indicators.sma(closes.slice(i, i+7), 7);
      if (v != null) ma.push(v);
    }
    if (ma.length >= 5) {
      const n = ma.length;
      const m2 = ma[n-5], m3 = ma[n-4], m4 = ma[n-2], m5 = ma[n-1];
      // 做多: MA7在上涨(最近上行) + 最新突然拐头向下(M5<M4且前一档由升转平/由升转降)
      if (pos.side === 'LONG') {
        // MA7上行过(m4>m3) 且 最新一根突然向下(m5<m4) → 到顶拐头, 平
        if (m4 > m3 && m5 < m4) {
          const pnl = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnl;
          return { action:'CLOSE', reason:`MA7上行后骤转下拐平多(到顶止盈,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
      } else if (pos.side === 'SHORT') {
        // MA7下行过(m4<m3) 且 最新一根突然向上(m5>m4) → 到底反转, 平
        if (m4 < m3 && m5 > m4) {
          const pnl = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnl;
          return { action:'CLOSE', reason:`MA7下行后骤转上拐平空(到底止盈,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
      }
    }
    // 2) 趋势反转兜底(MA7钝化时仍能平)
    try {
      ...
      if (tr && tr.dir !== 'FLAT') {
        if (pos.side === 'LONG' && tr.dir === 'DOWN') {
          const pnl = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnl;
          return { action:'CLOSE', reason:`趋势反转下拐平多(兜底,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
        if (pos.side === 'SHORT' && tr.dir === 'UP') {
          const pnl = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnl;
          return { action:'CLOSE', reason:`趋势反转上拐平空(兜底,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
      }
    } catch(e) {}
    return { action: 'HOLD' };
