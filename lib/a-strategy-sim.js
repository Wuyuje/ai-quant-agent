...
class AStrategySim {
  ...
  async checkTakeProfit(pos, klines) {
    // (大道至简v16·纯趋势止盈) — 最纯趋势: 只有该币15m趋势完全反转才平(吃满整波)
    // 做多: 15m EMA20 真正跌穿 EMA60(趋势由上升完全转下降) → 到顶, 平仓吃完全部升浪
    // 做空: 15m EMA20 真正升破 EMA60(趋势由下降完全转上升) → 到底, 平仓吃完全部跌浪
    // 不做MA7单拐头平(太灵敏会在趋势中途回调被洗), 只在趋势彻底反转时离场
    try {
      ...
      if (tr && tr.dir !== 'FLAT') {
        if (pos.side === 'LONG' && tr.dir === 'DOWN') {
          ...
          return { action:'CLOSE', reason:`趋势彻底反转下拐平多(吃满升浪到顶,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
        if (pos.side === 'SHORT' && tr.dir === 'UP') {
          const pnl = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnl;
          return { action:'CLOSE', reason:`趋势彻底反转上拐平空(吃满跌浪到底,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
      }
    } catch(e) {}
    return { action: 'HOLD' };
