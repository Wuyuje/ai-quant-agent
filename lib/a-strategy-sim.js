...
class AStrategySim {
  ...
  async checkSignal(klines, symbol) {
    ...
    const p7   = Indicators.sma(closes.slice(-8,-1),7);   // 上一根MA7
    const p25  = Indicators.sma(closes.slice(-26,-1),25); // 上一根MA25
    const p99  = Indicators.sma(closes.slice(-100,-1),99);// 上一根MA99
    ...
    // ---- 规则1做多: 三线低位合一 + MA7急转向上上穿25和99金叉 ----
    // 低位: MA7/25/99 在相对低位且相互靠近(三线合一)
    const lowZone  = ma7 < ma25*1.02 && ma99 <= ma25*1.02;
    if (lowZone && gap < 3.0 && ma7Up && ma7 > ma25 && ma7 > ma99 && p7 <= p25) {
      return { allowed:true, direction:'LONG', reason:`三线合一金叉做多: MA7(${ma7.toFixed(5)})上穿25(${ma25.toFixed(5)})/99(${ma99.toFixed(5)})`, confidence:0.7, atrPct:0.4 };
    }
    // ---- 规则5做空: MA7 高位下穿25和99死叉 ----
    const highZone = ma7 < ma25*1.02 && ma99 >= ma25*0.98;
    if (highZone && ma7Down && ma7 < ma25 && ma7 < ma99 && p7 >= p25) {
      return { allowed:true, direction:'SHORT', reason:`三线死叉做空: MA7(${ma7.toFixed(5)})下穿25(${ma25.toFixed(5)})/99(${ma99.toFixed(5)})`, confidence:0.7, atrPct:0.4 };
    }
    return { allowed:false, reason:`MA无三线信号(${ma7.toFixed(3)}/${ma25.toFixed(3)}/${ma99.toFixed(3)})` };
  }
