...
const CONFIG = {
  ...
  hardStopPct: 12,              // 绝对硬止损兜底(防ATR失效/极端行情, 浮亏≥12%强制平)
  atrStopMult: 2,               // ATR止损: 浮亏达2×ATR%止损(波动自适应, 低波动紧/高波动宽)
  trailingArmPct: 5,            // 移动止盈启用线
  trailingBackoffPct: 4,        // 移动止盈回撤线
...
class AStrategySim {
  ...
  // 从动态交易对读取(由refreshPairs按回测回报率选币扩充, 最多20只)
    try {
      // ★ 全市场选币: 从币安全市场USDT永续过滤流动性后选币(趋势可选币池)
      if (CONFIG.useFullMarket) {
        ...
        if (allCoins && allCoins.length) return [...allCoins];
      }
      // 后备: 动态交易对(候选池)
  ...
  async checkSignal(klines, symbol) {
    // (大道至简v13·最终修正) 顺币自身趋势 + 低买高卖 — 简化过严条件,保证能出信号
    // 做多: 币15m趋势向上/刚转上 + MA7回踩到近端低位区(回调) + 最新MA7转上 => 顺势低买
    // 做空: 币15m趋势向下/刚转下 + MA7反弹到近端高位区(冲高) + 最新MA7转下 => 顺势高卖
    if (!klines || klines.length < 80) return { allowed: false, reason: 'K线不足' };
    const closes = klines.map(k => k.close);
    // ⛔ 高波动小币过滤: 5m ATR% 过高的极端币(如仿盘/Meme)风险大, 不做(避免GIGGLE式拉盘反向)
    try {
      const atrAbs = Indicators.atr(klines, 14);
      const px = klines[klines.length-1].close || 1;
      const atrPct = px>0 ? atrAbs/px*100 : 0;
      const maxAtr = CONFIG.maxAtmPct || 15;   // 5m ATR% >15% 视为极端高波动, 拒
      if (atrPct > maxAtr) return { allowed:false, reason:`高波动过滤: 5m ATR${atrPct.toFixed(1)}%>${maxAtr}%` };
    } catch(e) {}
    const ma = [];
    ...
    if (ma.length < 12) return { allowed:false, reason:'MA7不足' };
    const n = ma.length;
    const base = Math.abs(ma[n-1] || 1);
    const curMA = ma[n-1];
    // 近端位置(0最低 1最高)
    const lb = ma.slice(-16);
    const mx = Math.max(...lb), mn = Math.min(...lb);
    const maRange = (mx - mn) || 1;
    const posRatio = (curMA - mn) / maRange;
    // 最新单段 + 前段
    const latest = (ma[n-1] - ma[n-2]) / base;
    const prevSeg = (ma[n-2] - ma[n-3]) / base;
    const turnMin = CONFIG.maTurnThreshold || 0.0006;
    // 币自身15m趋势方向
    let tdir = 'FLAT';
    try { const tr = await this._getSymbolTrend(symbol); if (tr) tdir = tr.dir; } catch(e) {}
    // 做多(低买): 币趋势向上(或持平) + MA7回踩低位 + 最新转上
    const lowBuy = (tdir === 'UP' || tdir === 'FLAT') && posRatio < 0.45 && latest > turnMin;
    // 做空(高卖): 币趋势向下(或持平) + MA7冲高 + 最新转下
    const highSell = (tdir === 'DOWN' || tdir === 'FLAT') && posRatio > 0.55 && latest < -turnMin;
    if (lowBuy) {
      return { allowed:true, direction:'LONG', reason:`低买(币${tdir} 回踩位${(posRatio*100).toFixed(0)}% 转上${(latest*100).toFixed(2)}%)`, confidence:0.72, atrPct:0.4 };
    }
    if (highSell) {
      return { allowed:true, direction:'SHORT', reason:`高卖(币${tdir} 冲高位${(posRatio*100).toFixed(0)}% 转下${(latest*100).toFixed(2)}%)`, confidence:0.72, atrPct:0.4 };
    }
    return { allowed:false, reason:`币${tdir} 位${(posRatio*100).toFixed(0)}% 最新${latest>0?'上':'下'}${(Math.abs(latest)*100).toFixed(3)}%` };
  }

  
  async checkTakeProfit(pos, klines) {
    // (大道至简v10·趋势版) 趋势反转或移动止盈才离场 → 让趋势跑, 不在中途回调被洗出去
    // 主要离场: 15m趋势反转(EMA20穿越EMA60, 与持仓方向相悖) → 平仓
    // 兜底: 5m MA7趋势反转(15m拉取失败也能兜住逆势仓, 防趋势反转但未平)
    try {
      // 1) 15m趋势反转离场
      const tr = await this._getSymbolTrend(pos.symbol);
      if (tr && tr.dir !== 'FLAT') {
        if (pos.side === 'LONG' && tr.dir === 'DOWN') {
          const pnl = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnl;
          return { action:'CLOSE', reason:`趋势反转下拐平多(顺转逆,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
        if (pos.side === 'SHORT' && tr.dir === 'UP') {
          const pnl = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnl;
          return { action:'CLOSE', reason:`趋势反转上拐平空(顺转逆,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
      }
    } catch(e) {}
    const pnlPct = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnlPct;
    // 1b) 5m MA7 顺逆势兜底: 若 15m 趋势未判出(FLAT), 用5分钟 MA7 明确反向也平(防逆势扛单)
    try {
      const closes = klines.map(k => k.close);
      const ma7 = [];
      for (let i = Math.max(0, closes.length-13); i <= closes.length-7; i++) { const v = Indicators.sma(closes.slice(i,i+7),7); if(v!=null) ma7.push(v); }
      if (ma7.length >= 5) {
        const mk = ma7[ma7.length-1], mp = ma7[ma7.length-2];
        // 做多却 MA7 明显转下(连续2根下行) → 平; 做空却 MA7 明显转上 → 平
        if (pos.side === 'LONG' && mk < mp && ma7[ma7.length-2] < ma7[ma7.length-3]) {
          return { action:'CLOSE', reason:`MA7连续下拐平多(5m兜底,${pnlPct>=0?'落盈':'止损'}${pnlPct.toFixed(1)}%)` };
        }
        if (pos.side === 'SHORT' && mk > mp && ma7[ma7.length-2] > ma7[ma7.length-3]) {
          return { action:'CLOSE', reason:`MA7连续上拐平空(5m兜底,${pnlPct>=0?'落盈':'止损'}${pnlPct.toFixed(1)}%)` };
        }
      }
    } catch(e) {}
    // 2) 分批止盈(1×ATR、2×ATR 各落袋1/3, 防利润回吐) + 移动止盈吃剩余趋势
    if (pnlPct > (pos._peakPnlPct||0)) pos._peakPnlPct = pnlPct;
    let atrPct = 0;
    try { const atrAbs = Indicators.atr(klines,14); const px = pos.currentPrice||1; atrPct = px>0?atrAbs/px*100:0; } catch(e) {}
    // 分批止盈进度 (0=没到1×ATR,1=已落袋1/3,2=已落袋2/3)
    const tpState = pos._tpState || 0;
    if (atrPct > 0) {
      if (tpState < 1 && pnlPct >= atrPct && pnlPct > 0) {
        pos._tpState = 1;
        return { action:'SCALE_OUT', pct: 0.33, reason:`分批止盈1: 盈利${pnlPct.toFixed(1)}%达1×ATR${atrPct.toFixed(1)}%,平1/3落袋` };
      }
      if (tpState < 2 && pnlPct >= atrPct*2 && pnlPct > 0) {
        pos._tpState = 2;
        return { action:'SCALE_OUT', pct: 0.33, reason:`分批止盈2: 盈利${pnlPct.toFixed(1)}%达2×ATR,再平1/3落袋` };
      }
    }
    // 3) 移动止盈: 剩余仓位吃趋势(峰值回撤)
    const armPct = CONFIG.trailingArmPct || 5;
    const backoffPct = CONFIG.trailingBackoffPct || 4;
    if (pos._peakPnlPct >= armPct && pnlPct <= pos._peakPnlPct - backoffPct && pnlPct > 0) {
      return { action:'CLOSE', reason:`移动止盈(峰值${pos._peakPnlPct.toFixed(1)}%回撤${(pos._peakPnlPct-pnlPct).toFixed(1)}%)锁${pnlPct.toFixed(1)}%` };
    }
    return { action: 'HOLD' };
  }
  ...
  checkStopLoss(pos, klines) {
    ...
    const atrMult = CONFIG.atrStopMult || 2;       // ATR止损倍数: 浮亏达2×ATR%止损(波动自适应)
    const hardSl  = CONFIG.hardStopPct || 12;      // 绝对硬止损兜底(防ATR异常/极端行情)
    let atrPct = 0;
    try {
      const atrAbs = Indicators.atr(klines, 14);
      const px = pos.currentPrice || 1;
      atrPct = px > 0 ? atrAbs / px * 100 : 0;     // ATR转成价格百分比
    } catch(e) {}
    // 1) ATR动态止损: 浮亏达 2×ATR% 止损(低波动止损紧, 高波动止损宽, 不被正常波动扫掉)
    if (atrPct > 0 && pnlPct <= -(atrMult * atrPct)) {
      return { action:'CLOSE', reason:`ATR止损: 浮亏${pnlPct.toFixed(1)}%达${(atrMult*atrPct).toFixed(1)}%(2×ATR${atrPct.toFixed(1)}%)` };
    }
    // 2) 绝对硬止损兜底(防止ATR失效时无限扛单)
    if (pnlPct <= -hardSl) return { action:'CLOSE', reason:`硬止损: 浮亏${pnlPct.toFixed(1)}%≥${hardSl}%` };
    // 3) 超时平仓(防深套)
    if (pos.openTime && (Date.now()-pos.openTime) > (CONFIG.maxHoldKlines||48)*5*60*1000) {
      const pnlLabel = pnlPct>=0?'锁利':'止损';
      return { action:'CLOSE', reason:`超时平仓${pnlLabel}(>${(CONFIG.maxHoldKlines||48)*5/60||4}h) ${pnlPct.toFixed(1)}%` };
    }
    return { action:'HOLD' };
  }
  ...
  async _scan() {
    ...
    for (const symbol of Object.keys(this.positions)) {
      ...
      try {
        ...
        // 分批止盈: 平掉 1/3 仓位落袋(剩余继续吃趋势)
        if (tp.action === 'SCALE_OUT') {
          try {
            const scalePct = tp.pct || 0.33;
            const scaleQty = pos.qty * scalePct;
            if (this.realTrading && scaleQty > 0) {
              const px = pos.currentPrice;
              const pnl = (pos.side==='LONG' ? (px-pos.entryPrice)/pos.entryPrice : (pos.entryPrice-px)/pos.entryPrice) * pos.leverage * 100;
              // 真实减仓1/3
              if (pos.side==='LONG') await this.api.closeLong(symbol, scaleQty, this.precisionMap).catch(()=>{});
              else await this.api.closeShort(symbol, scaleQty, this.precisionMap).catch(()=>{});
              pos.qty = pos.qty - scaleQty;
              pos.margin = pos.margin * (1 - scalePct);
              this._log(`✂️ ${symbol} ${tp.reason} (平${(scaleQty).toFixed(4)},剩${pos.qty.toFixed(4)}) 已实现~${pnl.toFixed(1)}%`);
            } else {
              // 模拟盘: 减仓不实际下单
              pos.qty = pos.qty * (1 - scalePct);
              pos.margin = pos.margin * (1 - scalePct);
              this._log(`✂️ [模拟] ${symbol} ${tp.reason} (剩${pos.qty.toFixed(4)})`);
            }
            this._saveState();
          } catch(e) { this._log(`⚠️ ${symbol} 分批止盈异常: ${e.message.slice(0,30)}`); }
          continue;
        }
    ...
    for (const symbol of symbols) {
      ...
      try {
        ...
        if (signal.allowed) {
          // checkSignal(v12)已内含币自身趋势方向(顺币趋势) + 回调位置(低买/高卖)
