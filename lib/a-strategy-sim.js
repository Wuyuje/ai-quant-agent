...
const CONFIG = {
  ...
  // 只交易这3个币
  tradeSymbols: ["KOMAUSDT","SKYAIUSDT","VICUSDT","PTBUSDT","VANRYUSDT","BLESSUSDT","HOMEUSDT","HFTUSDT"],  // 8只回测盈利(实时回测可调)  // 剔除闲置(BTC/BNB)与低收益(1000SATS)
  // MA7拐头策略
  maPeriod: 7,                  // MA7均线
...
class AStrategySim {
  ...
  // 只做固定3只币(BTC/BNB/ZEC)
  async selectSymbols() {
    return [...(CONFIG.tradeSymbols || ['KOMAUSDT','SKYAIUSDT','VICUSDT','PTBUSDT','VANRYUSDT','BLESSUSDT','HOMEUSDT','HFTUSDT'])];
  }
  ...
  async checkSignal(klines, symbol) {
    ...
    const ma = [];
    for (let i = Math.max(0, closes.length-15); i <= closes.length-7; i++) {
      const v = Indicators.sma(closes.slice(i, i+7), 7);
      if (v != null) ma.push(v);
    }
    if (ma.length < 7) return { allowed:false, reason:'MA不足' };
    const n = ma.length;
    const m1=ma[n-7], m2=ma[n-6], m3=ma[n-5], m4=ma[n-4], m5=ma[n-3], m6=ma[n-2], m7=ma[n-1];
    const flat = CONFIG.maFlatThreshold || 0.00035;
    // 前段趋势: 用 m1→m5 的累计方向(必须足够大=真趋势,非横盘)
    const priorTrend = (m5 - m1) / Math.abs(m1);      // 前段累计幅度
    const trendUp   = priorTrend >  flat*8;           // 前段明确上行
    const trendDown = priorTrend < -flat*8;           // 前段明确下行
    // 横盘硬性排除: 前段累计幅度太小 = 横盘震荡, 绝不开仓
    if (Math.abs(priorTrend) < flat*8) return { allowed:false, reason:`横盘震荡(前段${(priorTrend*100).toFixed(3)}% < 趋势阈值),不开仓` };
    // 夹角+趋势成形:
    // 做多: 前段明确下行 → MA7止跌急转向上(夹角) → 且当前连续上行(趋势已成形,m6<m7)
    const turnUp = m7 > m6;              // 当期向上
    const angleUp = m6 < m7 && m5 < m4;  // 夹角转折点(从下行转上行)
    if (trendDown && turnUp && angleUp && m7 > m6* (1+flat)) {
      return { allowed:true, direction:'LONG', reason:`MA7夹角+趋势成形做多(前段下行,转上,趋势确认)`, confidence:0.7, atrPct:0.4 };
    }
    // 做空: 前段明确上行 → MA7止涨急转向下(夹角) → 且当前连续下行(趋势成形)
    const turnDown = m7 < m6;
    const angleDown = m6 > m7 && m5 > m4;
    if (trendUp && turnDown && angleDown && m7 < m6*(1-flat)) {
      return { allowed:true, direction:'SHORT', reason:`MA7夹角+趋势成形做空(前段上行,转下,趋势确认)`, confidence:0.7, atrPct:0.4 };
    }
    return { allowed:false, reason:`无'夹角+趋势成形'信号(前段${(priorTrend*100).toFixed(3)}%)` };
  }
  ...
  checkTakeProfit(pos, klines) {
    ...
    if (pos.side==='LONG') {
      // 做多: MA7前段上行后急转向下夹角 → 平仓
      if (m3>m1 && turn<-flat*3 && m5<m4 && m4>m3) {
        return {action:'CLOSE', reason:`MA7向下夹角平多${pnlPct>=0?'落袋':'止损'} ${pnlPct.toFixed(1)}%`};
      }
    } else if (pos.side==='SHORT') {
      // 做空: MA7前段下行后急转向上夹角 → 平仓
      if (m3<m1 && turn>flat*3 && m5>m4 && m4<m3) {
        return {action:'CLOSE', reason:`MA7向上夹角平空${pnlPct>=0?'落袋':'止损'} ${pnlPct.toFixed(1)}%`};
      }
    }
    return {action:'HOLD'};
  }
  ...
  async _syncPositions() {
    ...
    try {
      ...
      const allowedSet = new Set(CONFIG.tradeSymbols || ['KOMAUSDT','SKYAIUSDT','VICUSDT','PTBUSDT','VANRYUSDT','BLESSUSDT','HOMEUSDT','HFTUSDT']);
