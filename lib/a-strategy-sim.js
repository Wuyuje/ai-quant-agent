...
const CONFIG = {
  ...
  minHoldMinutes: 20,          // 开仓保护期: 刚开仓20分钟内不平(防趋势反转误判平仓)
...
class AStrategySim {
  ...
  async checkTakeProfit(pos, klines) {
    // (大道至简v19·纯趋势离场 + 开仓保护) — 只在趋势真正反转才平, 且刚开仓不平(防误平)
    // 开仓保护期: 开仓后前 N 分钟内的趋势反转视为噪音(刚开仓的正常波动), 不触发平仓
    // 避免"开空1分钟就因EMA微动被误判趋势反转而平仓"的频繁超短线问题
    const minHoldMs = (CONFIG.minHoldMinutes || 20) * 60 * 1000;   // 刚开仓20分钟内不平
    const elapsed = pos.openTime ? (Date.now() - pos.openTime) : 0;
    if (elapsed < minHoldMs) return { action: 'HOLD' };            // 保护期内不平仓
    try {
      ...
      if (tr && tr.dir !== 'FLAT') {
        if (pos.side === 'LONG' && tr.dir === 'DOWN') {
          ...
          return { action:'CLOSE', reason:`趋势反转下拐平多单(到顶,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
        if (pos.side === 'SHORT' && tr.dir === 'UP') {
          const pnl = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnl;
          return { action:'CLOSE', reason:`趋势反转上拐平空单(到底,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
      }
    } catch(e) {}
    return { action: 'HOLD' };
