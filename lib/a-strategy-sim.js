...
class AStrategySim {
  ...
  async checkTakeProfit(pos, klines) {
    // (大道至简v14·趋势止盈) — 纯粹趋势交易: 方向对了就拿到趋势真正结束(最顶点/最低点)才平
    // 离场唯一核心: 这只币的15m趋势方向反转(EMA20穿越EMA60, 趋势见顶/见底) → 平仓
    // 不做分批落袋/过早移动止盈(会在趋势中途把好单洗掉/提前下车)
    try {
      const tr = await this._getSymbolTrend(pos.symbol);
      // 趋势已确立(非横盘)且与持仓方向相悖 → 趋势反转, 到顶/到底, 平仓落袋
      if (tr && tr.dir !== 'FLAT') {
        if (pos.side === 'LONG' && tr.dir === 'DOWN') {
          const pnl = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnl;
          return { action:'CLOSE', reason:`趋势到顶下拐平多(吃完整升浪,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
        if (pos.side === 'SHORT' && tr.dir === 'UP') {
          const pnl = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnl;
          return { action:'CLOSE', reason:`趋势到底上拐平空(吃完整跌浪,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
        }
      }
    } catch(e) {}
    return { action: 'HOLD' };
