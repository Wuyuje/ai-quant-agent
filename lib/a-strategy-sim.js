...
const CONFIG = {
  ...
  // 只交易这3个币
  tradeSymbols: ["PTBUSDT","BLESSUSDT","HOMEUSDT","NEIROUSDT"],  // 剔除闲置(BTC/BNB)与低收益(1000SATS)
  // MA7拐头策略
  maPeriod: 7,                  // MA7均线
...
class AStrategySim {
  ...
  // 只做固定3只币(BTC/BNB/ZEC)
  async selectSymbols() {
    return [...(CONFIG.tradeSymbols || ['PTBUSDT','BLESSUSDT','HOMEUSDT','NEIROUSDT'])];
  }
  ...
  async _syncPositions() {
    ...
    try {
      ...
      const allowedSet = new Set(CONFIG.tradeSymbols || ['PTBUSDT','BLESSUSDT','HOMEUSDT','NEIROUSDT']);
