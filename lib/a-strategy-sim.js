...
class AStrategySim {
  ...
  async checkTakeProfit(pos, klines) {
    // 高位/低位平仓(CYS式) — 做多涨到高位MA7拐头向下平, 做空跌到底位MA7拐头向上平
    // 参考CYS成功平仓: MA7涨到高位区(位77%) + 拐头向下 → 平多止盈
    ...
    // 相对开仓MA7涨跌幅%(确认真到了高位/低位, 不是开仓时就在高位)
    const movePct = (curMA - pos._entryMA) / (pos._entryMA||1) * 100;
    // MA7在近期区间的位置(0低 1高) — CYS是位77%(高位区)+拐头向下
    const hist = ma.slice(-40);
    const mx = Math.max(...hist), mn = Math.min(...hist);
    const range = (mx - mn) || 1;
    const posRatio = (curMA - mn) / range;
    const m4 = ma[n-2], m5 = ma[n-1];        // 最新拐头(拐头向下/向上)
    const highCut = CONFIG.takeProfitHighCut || 0.55;   // 高位区(位置>0.55)
    const lowCut  = CONFIG.takeProfitLowCut || 0.45;    // 低位区(位置<0.45)
    // ═══ 逆势平仓保护(优先): 持仓方向与MA7趋势明显相反(反向跑≥4%)即平 ═══
    const gotFallbackPct = CONFIG.reverseClosePct || 4;
    if (pos.side === 'LONG' && pos._entryMA && movePct <= -gotFallbackPct) {
      return { action:'CLOSE', reason:`逆势平多(做多但MA7跌${(-movePct).toFixed(1)}%≥${gotFallbackPct}%,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
    }
    if (pos.side === 'SHORT' && pos._entryMA && movePct >= gotFallbackPct) {
      return { action:'CLOSE', reason:`逆势平空(做空但MA7涨${movePct.toFixed(1)}%≥${gotFallbackPct}%,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
    }
    // CYS式止盈: 做多真涨上来(movePct>0) + MA7在高位区 + 最新拐头向下 → 高位平多
    if (pos.side === 'LONG') {
      if (pos._entryMA && movePct > 0 && posRatio > highCut && m5 < m4) {
        return { action:'CLOSE', reason:`高位平多(CYS式:MA7位${(posRatio*100).toFixed(0)}%拐头向下,涨${movePct.toFixed(1)}%,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
      }
    } else if (pos.side === 'SHORT') {
      // CYS式止盈: 做空真跌下来(movePct<0) + MA7在低位区 + 最新拐头向上 → 低位平空
      if (pos._entryMA && movePct < 0 && posRatio < lowCut && m5 > m4) {
        return { action:'CLOSE', reason:`低位平空(CYS式:MA7位${(posRatio*100).toFixed(0)}%拐头向上,跌${(-movePct).toFixed(1)}%,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
      }
    }
    return { action: 'HOLD' };
  }
