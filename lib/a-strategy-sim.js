...
class AStrategySim {
  ...
  // v7: BTC大盘趋势判断(带缓存,用于做空过滤)
  async _isBtcBullish() {
    const btc = CONFIG.btcTrendSymbol || 'BTCUSDT';
    if (this._btcCache && Date.now() - this._btcCacheTime < CONFIG.btcTrendCacheMs) {
      return this._btcCache;
    }
    try {
      ...
      if (klines && klines.length >= 60) {
        ...
        const bull = !!(ema20 && ema60 && ema20 > ema60);
        this._btcCache = bull;
        this._btcCacheTime = Date.now();
        return bull;
      }
    } catch(e) {}
    return false; // 获取失败默认不拦截(保守)
  }
  ...
  async checkSignal(klines, symbol) {
    ...
    // v7: 行情过滤 — BTC大盘普涨时禁止做空(避免逆势空单)
    // 在做空判断前先检查大盘趋势
    if (isDowntrend && CONFIG.btcTrendEnabled && CONFIG.btcBullDenyShort) {
      try {
        const btcBull = await this._isBtcBullish();
        if (btcBull) {
  ...
  checkStopLoss(pos, klines) {
    ...
    // 软止损+趋势确认: 空单浮亏≥5%(更敏感),多单浮亏≥6%,且趋势对持仓不利 → 提前止损
    const softLoss = pos.side === 'SHORT' ? Math.max(5, CONFIG.softLossPct - 1) : CONFIG.softLossPct;
    if (pnlPct <= -softLoss && klines && klines.length >= 60) {
      ...
      if (ema20 && ema60) {
        ...
        if ((pos.side === 'LONG' && !trendUp) || (pos.side === 'SHORT' && trendUp)) {
          ...
          return { action: 'CLOSE', reason: `趋势止损: 浮亏${pnlPct.toFixed(1)}%,EMA现${trendDir}不利${pos.side}(软${softLoss}%)` };
        }
      }
    }
    return { action: 'HOLD' };
