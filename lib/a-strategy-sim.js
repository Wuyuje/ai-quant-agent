...
class AStrategySim {
  ...
  async checkSignal(klines, symbol) {
    ...
    // 前段明确趋势
    const trendDown = prior < -0.0006;
    const trendUp   = prior >  0.0006;
    // 做多: 前段下行 + 斜率转正加速(s1<0,s2<0,s3>0,s4>s3) + 拐头幅度足
    if (trendDown && s1<0 && s2<0 && s3>0 && s4>s3 && turn > 0.0006) {
      return { allowed:true, direction:'LONG', reason:`MA7拐头夹角做多(斜率转正加速+拐幅${(turn*100).toFixed(2)}%)`, confidence:0.7, atrPct:0.4 };
    }
    // 做空: 前段上行 + 斜率转负加速 + 拐头幅度足
    if (trendUp && s1>0 && s2>0 && s3<0 && s4<s3 && turn < -0.0006) {
      return { allowed:true, direction:'SHORT', reason:`MA7拐头夹角做空(斜率转负加速+拐幅${(turn*100).toFixed(2)}%)`, confidence:0.7, atrPct:0.4 };
    }
    return { allowed:false, reason:`MA7无趋势夹角(前段${(prior*100).toFixed(3)}%)` };
  }
