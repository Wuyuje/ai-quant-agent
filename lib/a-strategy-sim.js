...
const CONFIG = {
  ...
  initialBalance: 1000,
  leverage: 3,
  maxPositions: 5,         // v16新: 最多持仓5只(按你的要求)
  ...
  klineInterval: '5m',
  klineLimit: 200,
  topN: 20,              // v16新: 全市场排名前20的币(按你要求)
  
  blacklist: [],            // v2: 清空黑名单,释放低波动币供三线合一策略使用
...
class AStrategySim {
  ...
  // ═══ AI选币+指标确认 ═══
  // (重写) 三线合一趋势追踪 — 5分钟K线
  // (重构) 三线合一趋势追踪 — 识别"横盘后首次突破"(避免持续误报)
  // (终版) 三线合一趋势追踪 — 完全按规则实现(5分钟K线)
  // (v2严格按规则) 三线合一趋势追踪 — 5分钟K线
  async checkSignal(klines, symbol) {
    if (!klines || klines.length < 100) return { allowed: false, reason: 'K线不足' };
    const closes = klines.map(k => k.close);
    const lastClose = closes[closes.length-1];
    const ma7 = Indicators.sma(closes.slice(-7),7);
    const ma25 = Indicators.sma(closes.slice(-25),25);
    const ma99 = Indicators.sma(closes.slice(-99),99);
    const p7   = Indicators.sma(closes.slice(-8,-1),7);
    const p25  = Indicators.sma(closes.slice(-26,-1),25);
    const p99  = Indicators.sma(closes.slice(-100,-1),99);
    if (ma7==null||ma25==null||ma99==null||p7==null||p25==null||p99==null) return { allowed:false, reason:'MA不足' };
    const gap = Math.max(Math.abs(ma7-ma25), Math.abs(ma25-ma99), Math.abs(ma7-ma99)) / lastClose * 100;
    const ma7Up   = ma7 > p7;    // MA7 急转向上
    const ma7Down = ma7 < p7;    // MA7 急转向下
    // ---- 规则1: 做多 —— 三线低位合一, MA7急转向上上穿25和99金叉 ----
    // 低位: 三线都在相对低位且相互靠近(MA99贴近MA25下方)
    const lowZone = ma99 <= ma25 * 1.005 && ma7 <= ma25 * 1.01; // 三线在低区(99在25下方或贴近)
    if (lowZone && gap < 1.5 && ma7Up && ma7 > ma25 && ma7 > ma99 && p7 <= p25 && ma7 - ma25 > 0) {
      return { allowed:true, direction:'LONG', reason:`三线合一金叉做多: MA7(${ma7.toFixed(6)})上穿25(${ma25.toFixed(6)})/99(${ma99.toFixed(6)})`, confidence:0.7, atrPct:0.4 };
    }
    // ---- 规则2: 做空 —— MA7 在高位下穿25和99死叉 ----
    // 高位: 三线在高位排列(MA25/MA99相对高),且此前MA7在MA25/MA99上方(多头)
    const highZone = ma7 > p7 ? false : (p7 >= p25 && p7 >= p99); // 上一根MA7在25和99上方(高位)
    if (highZone && ma7Down && ma7 < ma25 && ma7 < ma99 && p7 >= p25 && p7 >= p99) {
      // 真·死叉: 上一根MA7在25/99上方, 当前MA7下穿到下方, 且在高位
      return { allowed:true, direction:'SHORT', reason:`三线死叉做空: MA7(${ma7.toFixed(6)})高位下穿25(${ma25.toFixed(6)})/99(${ma99.toFixed(6)})`, confidence:0.7, atrPct:0.4 };
    }
    return { allowed:false, reason:`MA无三线信号(${ma7.toFixed(4)}/${ma25.toFixed(4)}/${ma99.toFixed(4)})` };
  }
  ...
  checkTakeProfit(pos, klines) {
    if (!klines || klines.length < 30) return { action: 'HOLD' };
    const closes = klines.map(k => k.close);
    const ma7 = Indicators.sma(closes.slice(-7), 7);
    const ma25 = Indicators.sma(closes.slice(-25), 25);
    const p7 = Indicators.sma(closes.slice(-8, -1), 7);
    const p25 = Indicators.sma(closes.slice(-26, -1), 25);
    if (ma7 == null || ma25 == null || p7 == null || p25 == null) return { action: 'HOLD' };
    const lastC = closes[closes.length-1];
    // 大阴线: 当前K线跌幅>1.5%(5分钟)
    const prevC = closes[closes.length-2];
    const bigBearish = prevC>0 && ((lastC-prevC)/prevC*100) <= -1.5;
    if (pos.side === 'LONG') {
      // 做多止盈: MA7急转向下+大阴线, 或 MA7下穿MA25死叉
      const ma7TurnDown = ma7 < p7; // MA7拐头向下
      const deathCross = p7 >= p25 && ma7 < ma25; // MA7下穿25
      if ((ma7TurnDown && bigBearish) || deathCross) {
        return { action: 'CLOSE', reason: `做多止盈: MA7${ma7.toFixed(4)}${deathCross?'下穿25死叉':'急转向下+大阴线'}` };
      }
    } else if (pos.side === 'SHORT') {
      // 做空止盈: MA7拐头向上 上穿MA25
      const ma7TurnUp = ma7 > p7;
      const goldenCross = p7 <= p25 && ma7 > ma25; // MA7上穿25
      if (ma7TurnUp && goldenCross) {
        return { action: 'CLOSE', reason: `做空止盈: MA7${ma7.toFixed(4)}上穿25金叉` };
      }
    }
    return { action: 'HOLD' };
  }
  ...
  checkStopLoss(pos, klines) {
    ...
    if (klines && klines.length >= 30) {
      ...
      const p7 = Indicators.sma(closes.slice(-8, -1), 7);
      const p25 = Indicators.sma(closes.slice(-26, -1), 25);
      if (ma7 != null && ma25 != null && p7 != null && p25 != null) {
        if (pos.side === 'LONG') {
          // 做多止损: MA7下穿MA25死叉(趋势转跌)
          if (p7 >= p25 && ma7 < ma25) {
            return { action: 'CLOSE', reason: `做多止损: MA7${ma7.toFixed(4)}下穿25` };
          }
        } else if (pos.side === 'SHORT') {
          // 做空止损: MA7上穿MA25金叉(趋势转涨)
          if (p7 <= p25 && ma7 > ma25) {
            return { action: 'CLOSE', reason: `做空止损: MA7${ma7.toFixed(4)}上穿25` };
          }
        }
      }
    }
    ...
    for (const symbol of symbols) {
      ...
      try {
        ...
        if (signal.allowed) {
          ...
          this._log(`🟢 ${symbol} ${signal.direction}三线信号: ${signal.reason}`);
