...
class AStrategySim {
  ...
  async checkSignal(klines, symbol) {
    ...
    const pp7  = Indicators.sma(closes.slice(-9, -2), 7);
    if (ma7==null||p7==null||pp7==null||p7<=0||pp7<=0) return { allowed:false, reason:'MA不足' };
    // MA7 拐头变化率
    const chg = (ma7 - p7) / p7;          // 当期变化率
    const prevChg = (p7 - pp7) / pp7;     // 上一期变化率(判断"急转"转折)
    const flat = CONFIG.maFlatThreshold || 0.00035;
    // 急转向上: 当前变化率显著>0, 且比上一期加速/转正(确认"突然拐头")
    const rapidUp = chg > flat && pp7 > 0 && (prevChg <= flat && pp7 <= closes[closes.length-3]*1.01);
    // 急转向下: 当前变化率显著<0
    const rapidDown = chg < -flat;
    // 横盘: 变化率绝对值很小 → 不开仓
    if (Math.abs(chg) < flat*0.5) return { allowed:false, reason:`横盘震荡(MA7变化${(chg*100).toFixed(4)}%),不开仓` };
    // 做多: MA7急拐向上(且此前未暴涨,是刚启动的拐头)
    if (rapidUp) {
      return { allowed:true, direction:'LONG', reason:`MA7急拐向上(${(chg*100).toFixed(3)}%)做多`, confidence:0.7, atrPct:0.4 };
    }
    // 做空: MA7急转向下 → 做空
    if (rapidDown) {
      return { allowed:true, direction:'SHORT', reason:`MA7急转向下(${(chg*100).toFixed(3)}%)做空`, confidence:0.7, atrPct:0.4 };
    }
    return { allowed:false, reason:`MA7拐头幅度(${(chg*100).toFixed(4)}%)不足` };
  }
  ...
  checkTakeProfit(pos, klines) {
    ...
    const ma7 = Indicators.sma(closes.slice(-7),7);
    const p7 = Indicators.sma(closes.slice(-8,-1),7);
    if (ma7==null||p7==null||p7<=0) return { action:'HOLD' };
    const chg=(ma7-p7)/p7;
    const flat=CONFIG.maFlatThreshold||0.00035;
    ...
    if (pos.side==='LONG') {
      // 做多: MA7拐头向下 → 止盈卖出
      if (chg < -flat) return { action:'CLOSE', reason:`做多止盈: MA7拐头向下(${(chg*100).toFixed(3)}%) ${pnlPct>=0?'落袋':'止损'}` };
    } else if (pos.side==='SHORT') {
      // 做空: MA7拐头向上 → 平仓卖出
      if (chg > flat) return { action:'CLOSE', reason:`做空平仓: MA7拐头向上(${(chg*100).toFixed(3)}%) ${pnlPct>=0?'落袋':'止损'}` };
    }
    return { action:'HOLD' };
  }

  // 止损兜底(防止极端行情/长时间横盘): 浮亏过大或持仓过久
