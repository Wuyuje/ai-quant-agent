...
const CONFIG = {
  ...
  // v18 长期底/顶判定
  minSpreadPct: 3,            // 行情区间幅度≥3%才有趋势(否则横盘跳过, 不选)
  bottomCut: 0.15,            // MA7位置<15% = 极底部(低买区)
  topCut: 0.85,              // MA7位置>85% = 极顶部(高卖区)
  maTurnRatio: 0.04,          // 拐头幅度=区间幅度的4%(动态, 过滤横盘微抖)
...
class AStrategySim {
  ...
  async checkSignal(klines, symbol) {
    ...
    const minSpread = CONFIG.minSpreadPct || 3;   // 区间需≥3%才有趋势可言(否则横盘)
    ...
    const turnMinAbs = (MA_range * (CONFIG.maTurnRatio || 0.04)); // 拐头幅度=区间幅度的4%(动态,不固定%数)
    const baseX = Math.abs(curMA) || 1;
    // ── 只在极底部或极顶部 + 明确拐头才开 ──
    const bottomCut = CONFIG.bottomCut || 0.15;   // 位置<15% = 极底部区
    const topCut    = CONFIG.topCut || 0.85;      // 位置>85% = 极顶部区
    const atBottom = posRatio < bottomCut;
    const atTop    = posRatio > topCut;
    // 做多: 极底部 + MA7刚转上拐
    const lowBuy = atBottom && latest > turnMinAbs && latest > 0 && prevSeg <= 0;
    // 做空: 极顶部 + MA7刚转下拐
    const highSell = atTop && latest < -turnMinAbs && latest < 0 && prevSeg >= 0;
    if (lowBuy) {
