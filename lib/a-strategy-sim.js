...
const CONFIG = {
  ...
  maTurnThreshold: 0.0005,      // 保留兼容
  maTurnAngleDeg: 8,            // 拐头角度门槛≥8°(单段MA7变化≥近端波幅~14%), 角度够陡才认开仓
  maPriorPersist: 0.0008,       // 保留兼容
  hardStopPct: 8,               // 硬止损: 浮亏≥8%强制平仓(MA7拐头平仓优先,此兜底防跳空)
  trailingArmPct: 8,            // 移动止盈启用线: 利润≥8%才启用(避免小波动止盈)
  trailingBackoffPct: 2,        // 移动止盈回撤线: 从峰值回撤≥2%即锁利(不等MA7滞后下拐)
...
class AStrategySim {
  ...
  async checkSignal(klines, symbol) {
    // (大道至简v6) MA7拐头角度+确定性确认 — 上拐做多/下拐做空, 宁少开不逆向
    // 拐头“角度”: 单根MA7变化 vs 近端MA7波幅 ⇒ atan(单段/波幅)算真实角度(度)
    // 只有“有角度的陡峭拐头”(非平缓蠕动/横盘) + 2根同向加速 + 前段反向趋势 + 现价确认才开仓
    ...
    const n = ma.length;
    // 近端MA7波幅(角度归一尺度, 低价/高价币统一)
    const near = ma.slice(-6);
    const range = Math.max(...near) - Math.min(...near);
    if (range === 0) return { allowed:false, reason:'MA7横盘(无波幅)' };
    // 斜率序列
    const sl = [];
    for (let q=1;q<n;q++) sl.push(ma[q]-ma[q-1]);
    if (sl.length < 4) return { allowed:false, reason:'MA7斜率不足' };
    const s3=sl[sl.length-2], s4=sl[sl.length-1];
    // 拐头角度(度): atan(最新单段斜率/近端波幅)
    const angleDeg = Math.atan(Math.abs(s4)/range) * 180 / Math.PI;
    // 前段方向(拐头前3根) — 须反向, 排除横盘蠕动
    const prior = ma[n-3] - ma[n-6];
    const price = closes[closes.length-1];
    // 上拐/下拐 2根同向 + 最新加速(陡拐确认)
    const upK = s3>0 && s4>0 && Math.abs(s4)>=Math.abs(s3);
    const dnK = s3<0 && s4<0 && Math.abs(s4)>=Math.abs(s3);
    // 门槛
    const angleMin = CONFIG.maTurnAngleDeg || 8;   // 角度≥8°(单段≥近端波幅~14%, 足够陡)
    const angOk    = angleDeg >= angleMin;
    const priorDn  = prior < 0;   // 前段下行(做多反转前提)
    const priorUp  = prior > 0;   // 前段上行(做空反转前提)
    const priceUp  = price > ma[n-1];  // 现价站上MA7(做多确认)
    const priceDn  = price < ma[n-1];  // 现价跌破MA7(做空确认)
    // ── 做多: 前段下行 → 有角度的上拐(2根同向加速) → 现价确认 ──
    if (priorDn && upK && angOk && priceUp) {
      return { allowed:true, direction:'LONG', reason:`MA7上拐做多(角度${angleDeg.toFixed(1)}° 前段${(prior*100).toFixed(2)}%下行后陡拐)`, confidence:0.75, atrPct:0.4 };
    }
    // ── 做空: 前段上行 → 有角度的下拐(2根同向加速) → 现价确认 ──
    if (priorUp && dnK && angOk && priceDn) {
      return { allowed:true, direction:'SHORT', reason:`MA7下拐做空(角度${angleDeg.toFixed(1)}° 前段${(prior*100).toFixed(2)}%上行后陡拐)`, confidence:0.75, atrPct:0.4 };
    }
    return { allowed:false, reason:`MA7拐头角度不足(角度${angleDeg.toFixed(1)}°, 需≥${angleMin}°)` };
  }
  ...
  checkTakeProfit(pos, klines) {
    ...
    const backoffPct = CONFIG.trailingBackoffPct || 2; // 从峰值回撤≥2%即锁利(不等MA7滞后)
    if (pos.side==='LONG') {
