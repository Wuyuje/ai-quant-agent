...
const CONFIG = {
  ...
  maxPositions: 3,             // 同时只持精选约3仓(趋势策略精挑, 避免全市场撒网频繁双向乱开)
...
class AStrategySim {
  ...
  async checkSignal(klines, symbol) {
    ...
    // 币自身15m趋势: 必须强趋势(ADX≥18)且方向明确才开(过滤FLAT/弱趋势假信号, 止散弹枪误开)
    let tdir = 'FLAT', tstrong = false;
    try { const tr = await this._getSymbolTrend(symbol); if (tr) { tdir = tr.dir; tstrong = tr.strong; } } catch(e) {}
    // 只在币的强趋势方向内、回调到低/高位才进: 做多需UP强, 做空需DOWN强(FLAT/弱趋势绝不开)
    const lowBuy = tdir === 'UP' && tstrong && posRatio < 0.45 && latest > turnMin;
    // 做空(高卖): 币强DOWN + MA7冲高 + 最新转下
    const highSell = tdir === 'DOWN' && tstrong && posRatio > 0.55 && latest < -turnMin;
    if (lowBuy) {
  ...
  async checkTakeProfit(pos, klines) {
    // (大道至简v17·纯趋势离场唯一) — 除了趋势反转离场, 其它一律删除
    // 做多: 该币15m趋势完全反转(EMA20跌穿EMA60, 由升转降=趋势到顶) → 平仓
    // 做空: 该币15m趋势完全反转(EMA20升破EMA60, 由降转升=趋势到底) → 平仓
    // 不做任何止损/移动止盈/MA7单拐头 — 只等趋势真正反转, 吃完整波
    try {
