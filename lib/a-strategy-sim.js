...
const CONFIG = {
  ...
  // 只交易这3个币
  tradeSymbols: ["1000RATSUSDT","BLESSUSDT","VICUSDT","PTBUSDT","BICOUSDT","HFTUSDT","1000SATSUSDT","BANKUSDT"],  // 8只在案(v2夹角回测盈利,可动态回测更新),  // 8只回测盈利(实时回测可调)  // 剔除闲置(BTC/BNB)与低收益(1000SATS)
  // MA7拐头策略
  maPeriod: 7,                  // MA7均线
...
class AStrategySim {
  ...
  // 只做固定3只币(BTC/BNB/ZEC)
  async selectSymbols() {
    return [...(CONFIG.tradeSymbols || ['1000RATSUSDT','BLESSUSDT','VICUSDT','PTBUSDT','BICOUSDT','HFTUSDT','1000SATSUSDT','BANKUSDT'])];
  }
  ...
  async checkSignal(klines, symbol) {
    ...
    // 计算最近约15个MA7点(用足够历史看整体走势)
    const ma = [];
    for (let i = closes.length-22; i <= closes.length-7; i++) {
      const v = Indicators.sma(closes.slice(i, i+7), 7);
      if (v != null) ma.push(v);
    }
    if (ma.length < 9) return { allowed:false, reason:'MA7不足' };
    const n = ma.length;
    // 用MA7的"斜率序列"判断方向和拐头——用相对变化,对低价币用足够阈值
    // 前段: ma[n-9]~ma[n-4] 的斜率(看是否走出明确同向趋势)
    const seg1 = ma[n-9], seg2 = ma[n-7], seg3 = ma[n-5], seg4 = ma[n-4];
    const a = seg4 - seg1;   // 前段累计变化(带符号)
    const base = Math.abs(seg1 || 1);
    const priorRate = a / base;   // 前段相对幅度
    // 拐头: 最近4点(ma[n-4]~ma[n-1])是否与前端反向
    const b = ma[n-1] - seg4;  // 拐头后累计变化(带符号)
    // 夹角判定: 前端方向 + 拐头反向, 且拐头幅度 ≥ 前端幅度的一定比例(真夹角,非抖动)
    // 做多夹角: 前段明确向下(a<0) + 拐头向上(b>0) + 拐头力度足
    if (priorRate < -0.0010 && b > 0 && b >= Math.abs(a) * 0.3) {
      // 且当前仍在向上(ma[n-1]>ma[n-2]>ma[n-3])
      if (ma[n-1] > ma[n-2] && ma[n-2] > ma[n-3]) {
        return { allowed:true, direction:'LONG', reason:`MA7拐头夹角做多(前段${(priorRate*100).toFixed(2)}%→拐+/${(b/base*100).toFixed(2)}%)`, confidence:0.7, atrPct:0.4 };
      }
    }
    // 做空夹角: 前段明确向上 + 拐头向下 + 力度足
    if (priorRate > 0.0010 && b < 0 && Math.abs(b) >= Math.abs(a) * 0.3) {
      if (ma[n-1] < ma[n-2] && ma[n-2] < ma[n-3]) {
        return { allowed:true, direction:'SHORT', reason:`MA7拐头夹角做空(前段+${(priorRate*100).toFixed(2)}%→拐-/${(Math.abs(b)/base*100).toFixed(2)}%)`, confidence:0.7, atrPct:0.4 };
      }
    }
    return { allowed:false, reason:`MA7无拐头夹角(前段${(priorRate*100).toFixed(3)}%拐头${(b/base*100).toFixed(3)}%)` };
  }
  ...
  async _syncPositions() {
    ...
    try {
      ...
      const allowedSet = new Set(CONFIG.tradeSymbols || ['1000RATSUSDT','BLESSUSDT','VICUSDT','PTBUSDT','BICOUSDT','HFTUSDT','1000SATSUSDT','BANKUSDT']);
