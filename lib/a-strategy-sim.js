...
const CONFIG = {
  ...
  longLeverage: 12,             // 做多高杠杆(加大)
  longMarginPct: 0.10,          // 做多单次用10%资金(加大)
  // 做空: 做多平仓后趋势形成MA7拐向下 → 做空低杠杆; MA7拐向上平仓
  shortLeverage: 5,             // 做空杠杆(加大)
  shortMarginPct: 0.10,         // 做空单次用10%资金(加大)
...
class AStrategySim {
  ...
  async checkTakeProfit(pos, klines) {
    // 趋势止盈(防振出): 要趋势拐头止盈, 但不要太紧被小震荡振出
    // 平多: MA7趋势由升转降(连续3根反向=趋势明显反转) → 平多(吃完上升)
    // 平空: MA7趋势由降转升(连续3根反向=趋势明显反转) → 平空(吃完下降)
    // 连续3根反向=对震荡更钝, 不会因1-2根小回调被振出; 保留吃完趋势后及时止盈
    ...
    if (ma.length < 14) return { action:'HOLD' };
    const n = ma.length;
    const pnlPct = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnlPct;
    // MA7趋势反转判定: 连续3根反向(对震荡钝化, 不被1-2根小回调振出)
    const dn3 = ma[n-1]<ma[n-2] && ma[n-2]<ma[n-3] && ma[n-3]<ma[n-4];   // 连续3根向下(做多反转)
    const up3 = ma[n-1]>ma[n-2] && ma[n-2]>ma[n-3] && ma[n-3]>ma[n-4];   // 连续3根向上(做空反转)
    if (pos.side === 'LONG' && dn3) {
      return { action:'CLOSE', reason:`趋势拐头平多止盈(MA7连续3根向下,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
    }
    if (pos.side === 'SHORT' && up3) {
      return { action:'CLOSE', reason:`趋势拐头平空止盈(MA7连续3根向上,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
    }
    return { action: 'HOLD' };
  }
  ...
  async _simOpen(symbol, direction, signal) {
    ...
    const longBase = CONFIG.longLeverage || 12, shortBase = CONFIG.shortLeverage || 5;
    const adaptive = (baseLev, b) => {
      if (b < 30) return Math.max(3, Math.round(baseLev * 0.6));   // 资金<30U, 杠杆×0.6
      if (b < 100) return baseLev;                                  // 30-100U, 标准
      if (b < 300) return Math.min(baseLev + 2, 20);               // 100-300U, +2x(封顶20)
      return Math.min(baseLev + 3, 20);                            // >300U, +3x(封顶20)
    };
    let leverage = (direction === 'LONG' ? adaptive(longBase, bal) : adaptive(shortBase, bal));
    // 按波动率(ATR%)压制杠杆: 高波动币(小币/行情剧烈)杠杆调低, 防强平
    try {
      const kl = signal._klines || [];
      if (kl.length > 14) {
        const c=kl.map(k=>k.close), h=kl.map(k=>k.high), l=kl.map(k=>k.low);
        let trSum=0; for(let i=1;i<kl.length;i++){ const tr=Math.max(h[i]-l[i], Math.abs(h[i]-c[i-1]), Math.abs(l[i]-c[i-1])); trSum+=tr; }
        const atrPct=(trSum/(kl.length-1))/(Math.abs(c[c.length-1])||1)*100;
        if (atrPct > 2.0) leverage = Math.max(2, Math.round(leverage*0.5));   // 高波动ATR>2%, 杠杆减半
        else if (atrPct > 1.0) leverage = Math.max(2, Math.round(leverage*0.75)); // 中波动, 杠杆×0.75
      }
    } catch(e) {}
