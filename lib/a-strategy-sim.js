...
class AStrategySim {
  ...
  async checkTakeProfit(pos, klines) {
    // 趋势止盈(防振出): 要吃到完整趋势, 小回调(3根以下MA7反向)不平仓
    // 平多: MA7连续5根向下(幅度够深=趋势真反转) → 平多(吃完上升)
    // 平空: MA7连续5根向上(趋势真反转) → 平空(吃完下降)
    // 5根反向比3根更钝: 震荡小回调(1-3根)不平, 只有真反转(5根持续反向)才平, 吃满趋势不被振出
    ...
    if (ma.length < 16) return { action:'HOLD' };
    const n = ma.length;
    const pnlPct = this._calcPnlPct(pos, pos.currentPrice); pos._lastPnlPct = pnlPct;
    // 连续5根反向(真趋势反转, 对震荡钝化不被振出)
    const dn5 = ma[n-1]<ma[n-2] && ma[n-2]<ma[n-3] && ma[n-3]<ma[n-4] && ma[n-4]<ma[n-5] && ma[n-5]<ma[n-6];
    const up5 = ma[n-1]>ma[n-2] && ma[n-2]>ma[n-3] && ma[n-3]>ma[n-4] && ma[n-4]>ma[n-5] && ma[n-5]>ma[n-6];
    if (pos.side === 'LONG' && dn5) {
      return { action:'CLOSE', reason:`趋势反转平多止盈(MA7连续5根向下,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
    }
    if (pos.side === 'SHORT' && up5) {
      return { action:'CLOSE', reason:`趋势反转平空止盈(MA7连续5根向上,落${pnlPct>=0?'盈':'损'}${pnlPct.toFixed(1)}%)` };
    }
    return { action: 'HOLD' };
  }
  ...
  checkStopLoss(pos, klines) {
    ...
    // 插针保护: 浮亏快速急扩大(较上次巡检跌超25%且已亏25%+)→立即平, 不等强平(DODOX式插针)
    if (pos._lastChkPnl !== undefined && pnl <= -25 && (pnl - pos._lastChkPnl) <= -25) {
      return { action:'CLOSE', reason:`⚠️插针保护平仓(浮亏${pnl.toFixed(1)}%急扩大${(pos._lastChkPnl-pnl).toFixed(0)}%,防强平)` };
    }
    pos._lastChkPnl = pnl;
    if (pnl <= -liqProtect) {
