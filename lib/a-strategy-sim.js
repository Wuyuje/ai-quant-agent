...
const CONFIG = {
  ...
  peakBackoffPct: 0.8,        // 抓最高点平多/最低点平空: 新高回落/新低回升≥0.8%判定到顶/到底(接近趋势极值)
...
class AStrategySim {
  ...
  async checkTakeProfit(pos, klines) {
    // 吃趋势最高点/最低点 — 平仓在接近趋势顶点/底点的位置
    // 平多(最高点): 持仓价创本轮新高后, 从新高回落超过阈值 → 判定"到顶", 在最高点附近平多(低买高卖吃满)
    // 平空(最低点): 持仓价创本轮新低后, 从新低反弹超过阈值 → 判定"到底", 在最低点附近平空(高卖低买吃满)
    const price = parseFloat(pos.currentPrice) || 0;
    if (!klines || klines.length < 12 || !price) return { action:'HOLD' };
    // 追踪持仓期间的最高/最低价(pos._top: 多头最高, pos._bottom: 空头最低)
    if (pos.side === 'LONG') {
      if (!pos._top) pos._top = pos.entryPrice;
      if (price > pos._top) pos._top = price;   // 更新持仓最高价
      // 从最高价回落 ≥ peakBackoff% → 到顶, 平多
      const backoff = CONFIG.peakBackoffPct || 0.8;
      if (pos._top > pos.entryPrice && (pos._top - price) / pos._top * 100 >= backoff) {
        const pnl = this._calcPnlPct(pos, price); pos._lastPnlPct = pnl;
        return { action:'CLOSE', reason:`最高点平多(高点${pos._top.toFixed(4)},回落${(((pos._top-price)/pos._top)*100).toFixed(1)}%,吃满上升,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
      }
    } else if (pos.side === 'SHORT') {
      if (!pos._bottom) pos._bottom = pos.entryPrice;
      if (price < pos._bottom) pos._bottom = price;  // 更新持仓最低价
      // 从最低点回升 ≥ peakBackoff% → 到底, 平空
      const backoff = CONFIG.peakBackoffPct || 0.8;
      if (pos._bottom < pos.entryPrice && (price - pos._bottom) / pos._bottom * 100 >= backoff) {
        const pnl = this._calcPnlPct(pos, price); pos._lastPnlPct = pnl;
        return { action:'CLOSE', reason:`最低点平空(低点${pos._bottom.toFixed(4)},回升${(((price-pos._bottom)/pos._bottom)*100).toFixed(1)}%,吃满下降,落${pnl>=0?'盈':'损'}${pnl.toFixed(1)}%)` };
      }
    }
    return { action: 'HOLD' };
  }
