...
async function analyzeCoin(coin) {
  ...
  const [k5, k15] = await Promise.all([
    getK(coin, '5m', 250), getK(coin, '15m', 100),
  ]);
  ...
  const n5 = ma5.length;
  // 位置判定: 用近200根5分钟K线(≈16小时)确定低位/高位, 比100根(8小时)更可靠, 避免短线波动误判
  const POS_LOOKBACK = 200;   // 用近200根确定低位/高位(约16小时)
  const lbMA = ma5.length > POS_LOOKBACK ? ma5.slice(-POS_LOOKBACK) : ma5;
  const pos5 = (ma5[n5-1] - Math.min(...lbMA)) / ((Math.max(...lbMA)-Math.min(...lbMA))||1);  // 位置0低1高(相对近16h)
