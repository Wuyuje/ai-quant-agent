...
async function analyzeCoin(coin) {
  ...
  // ── 强主判定(多级别+动量+幅度确认, 方向绝不反) ──
  const turnMinPct = 0.05;   // 拐头幅度≥0.05%(价%)
  // 做空(高卖): 高位(pos5>0.6) + MA7向下加速 + 拐头幅度足 + (15m偏空 且 1h偏空或平) → SHORT
  if (pos5 > 0.6 && turnDn5 && turnPct>=0.08 && trendDn15 && (trendDn1h || Math.abs(e20_15-e60_15)<0.5)) {
    return { coin, direction:'SHORT', confidence:0.85, reason:`高位(${Math.round(pos5*100)}%)反转向下+15m/1h空=高卖做空` };
  }
  // 做多(低买): 低位(pos5<0.4) + MA7向上加速 + 拐头幅度足 + (15m偏多 且 1h偏多或平) → LONG
  if (pos5 < 0.4 && turnUp5 && turnPct>=0.08 && trendUp15 && (trendUp1h || Math.abs(e20_15-e60_15)<0.5)) {
    return { coin, direction:'LONG', confidence:0.85, reason:`低位(${Math.round(pos5*100)}%)反转向上+15m/1h多=低买做多` };
  }
  // 次强(位置极端+拐头加速, 幅度大): 给方向但置信略低
  if (pos5 < 0.3 && turnUp5 && turnPct>=0.08 && trendUp15) return { coin, direction:'LONG', confidence:0.7, reason:`低位(${Math.round(pos5*100)}%)+15m多+=低买` };
  if (pos5 > 0.7 && turnDn5 && turnPct>=0.08 && trendDn15) return { coin, direction:'SHORT', confidence:0.7, reason:`高位(${Math.round(pos5*100)}%)+15m空=高卖` };
  return { coin, direction:'NONE', confidence:0, reason:`未达底/顶反转(位${Math.round(pos5*100)}% 拐幅${turnPct.toFixed(2)}% 波动${atr.toFixed(2)}%)` };
}
