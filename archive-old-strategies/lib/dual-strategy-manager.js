...
class DualStrategyManager {
  constructor(opts = {}) {
    ...
    // 共享行情缓存（百万用户：公开行情所有用户共用一份，减少API调用）
    this.sharedMarket = new SharedMarket();
    // 账户级总风控（防多引擎叠加超载/防强平）
    this.accountGuard = new AccountCapacityGuard({ maxTotalLeverage: 5, maxSymbolLeverage: 2 });
    ...
    this.ADMIN_WALLETS = [
      ...
      '0xc6dbb4cd3b6a12068c7388248da2bd32df7ef9b7', // 白名单用户,免算力费
    ];
  ...
  async _ensureEngine(wallet, apiKey, apiSecret, gatesFeeLow) {
    ...
    // v3: 统一用小写wallet做key,防止大小写不一致导致重复创建
    const walletKey = wallet.toLowerCase();
    
    // BB引擎
    if (!this._bbEngines[walletKey]) {
      const bb = new BBStrategy(apiKey, apiSecret, wallet);
      bb.gatesFeePaused = gatesFeeLow;
      bb._userDB = this.userDB;
      this._bbEngines[walletKey] = bb;
      this._injectSharedMarket(bb);  // 共享行情(选币ticker): 所有用户共用一份
      // v3: 各用户错开5秒启动,防止同时请求API
      const delay = Object.keys(this._bbEngines).length * 5000;
      setTimeout(() => bb.start().catch(e => this._log(`❌ ${wallet.slice(0,10)} BB启动失败: ${e.message}`)), delay);
      if (this._cycleCount <= 5) this._log(`✅ ${wallet.slice(0,10)}... BB引擎启动(延迟${delay}ms)`);
    } else {
      this._bbEngines[walletKey].gatesFeePaused = gatesFeeLow;
    }
    
    // 趋势引擎(延迟30秒错峰启动)
    if (!this._trendEngines[walletKey]) {
      const trend = new TrendStrategy(apiKey, apiSecret, wallet);
      trend.gatesFeePaused = gatesFeeLow;
      trend._userDB = this.userDB;
      this._trendEngines[walletKey] = trend;
      this._injectSharedMarket(trend);  // 共享行情(选币ticker): 所有用户共用一份
      // v3: 趋势引擎延迟30秒+各用户错开5秒
      const trendDelay = 30000 + Object.keys(this._trendEngines).length * 5000;
      setTimeout(() => {
        trend.start().catch(e => this._log(`❌ ${wallet.slice(0,10)} 趋势启动失败: ${e.message}`));
      }, trendDelay);
      if (this._cycleCount <= 5) this._log(`✅ ${wallet.slice(0,10)}... 趋势引擎启动`);
    } else {
      this._trendEngines[walletKey].gatesFeePaused = gatesFeeLow;
    }
    
    // 互相注入仓位(避免接管对方仓位)
    this._bbEngines[walletKey]?.setTrendPositions?.(this._trendEngines[walletKey]?.positions || {});
    this._trendEngines[walletKey]?.setBBPositions?.(this._bbEngines[walletKey]?.positions || {});
  }

  /**
   * 注入共享行情到引擎 —— 百万用户核心优化
   * 选币用 getAllTickers(全市场ticker)是最大的API消耗，所有用户共用一份缓存。
   * 只覆盖公开行情，私有API(下单/余额/持仓/划转)仍走各用户自己的BinanceAPI，完全隔离。
   * 失败自动回落各引擎自己的API；完整可回退。
   */
  _injectSharedMarket(engine) {
    if (!engine || !engine.api || !this.sharedMarket) return;
    const shared = this.sharedMarket;
    // 注入账户级总风控（跨引擎共享同一账户容量判断，防叠加超载）
    if (this.accountGuard) {
      engine.accountGuard = this.accountGuard;
    }
    // 标记已注入，避免重复覆盖
    if (engine.__sharedInjected) return;
    engine.__sharedInjected = true;

    // 覆盖 getAllTickers（选币）：优先共享缓存，失败回落引擎自己的API
    const origGetAllTickers = engine.api.getAllTickers && engine.api.getAllTickers.bind(engine.api);
    engine.api.getAllTickers = async () => {
      try {
        const tickers = await shared.getAllTickers();
    ...
    // 覆盖 getExchangeInfo（精度表，1小时缓存）：所有用户共用
    const origGetExchangeInfo = engine.api.getExchangeInfo && engine.api.getExchangeInfo.bind(engine.api);
    engine.api.getExchangeInfo = async () => {
      try {
  ...
  _watchdog(activeUsers) {
    // v3: 禁止看门狗重启引擎 — 防止引擎堆积导致API被封
    // 引擎停止时由_cycle重新创建,不需要看门狗重启
  }
