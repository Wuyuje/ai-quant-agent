...
class BBEngine {
  ...
  // ═══ 自愿打赏模式：不自动扣费，只记账+余额保护 ═══
  // 原自动链上转账已禁用，用户需主动调用 /api/vault/tip 发起打赏
  async _tryBatchFeeTransfer(walletKey) {
    ...
    if (totalFee < BBEngine.FEE_CONFIG.FEE_THRESHOLD) {
      this._log(`📊 ${walletKey.slice(0,10)} 待打赏累计 $${totalFee.toFixed(2)} < $${BBEngine.FEE_CONFIG.FEE_THRESHOLD} 阈值，继续积累`);
      return;
    }

    // 自愿打赏模式：不主动链上转账，只做余额保护
    // 如果用户余额不足，暂停开新仓（防止策略非预期亏损）
    if (this.userDB) {
      const user = this.userDB.get(walletKey.toLowerCase()) || {};
      const balance = user.gatesFeeBalance || 0;
      if (totalFee > balance && !user.gatesFeeLow) {
        this.userDB.set(walletKey.toLowerCase(), {
          ...user,
          gatesFeeLow: true,
          gatesFeeApproved: true,
        });
        this._log(`⚠️ ${walletKey.slice(0,10)} 待打赏 $${totalFee.toFixed(2)} 超过余额 $${balance.toFixed(2)}，暂停开新仓（自愿打赏模式）`);
      }
    }
    // pending 保留不别除，等用户主动打赏后才清除
  }

  // ═══ 自愿打赏入口：用户主动调用 /api/vault/tip 时调用此方法 ═══
  async processTip(walletKey, amount) {
    const pending = this._feeState.pending[walletKey] || [];
    if (pending.length === 0) {
    ...
    if (!traderPrivateKey) return { ok: false, msg: 'TRADER_PRIVATE_KEY 未配置' };
    ...
    const tipWei = ethers.parseUnits(tipAmount.toFixed(6), 18);
    if (BigInt(traderBal) < tipWei) {
      return { ok: false, msg: `Trader钱包USDT不足 ($${Number(traderBal)/1e18})` };
    }
    ...
    try {
      if (platformAmount > 0) {
        ...
        this._log(`💰 ${walletKey.slice(0,10)} 用户自愿打赏服务费 $${platformAmount.toFixed(2)} → ${PLATFORM_WALLET.slice(0,10)}...`);
      ...
      if (this.userDB) {
        ...
        this._log(`✅ ${walletKey.slice(0,10)} 自愿打赏成功 $${tipAmount.toFixed(2)}，余额 $${newBalance.toFixed(2)}`);
      }

      this._saveFeeState();
      return { ok: true, tipAmount, txs, msg: '打赏成功' };
    } catch (e) {
      this._log(`❌ ${walletKey.slice(0,10)} 自愿打赏失败: ${e.message.slice(0,100)}`);
