...
class DexTrader {
  ...
  // ═══ DEX 盖茨费记账模式：仅记账，不自动扣费（自愿打赏模式） ═══
  // 自愿打赏模式：费用只记账不自动转账，用户需主动调用 /api/vault/tip 发起打赏
  // 保留 gatesFeeLow 余额不足保护，避免策略非预期亏损
  async _collectGatesFeeBookkeeping(wallet, bscWallet) {
    ...
    // 自愿打赏模式：不再自动链上转账，只更新用户 pending 记录供展示
    // 余额检查仍然生效：如果待扣 > 余额，暂停开新仓以保护用户
    if (this.userDB) {
      const user = this.userDB.get(wallet) || {};
      const balance = user.gatesFeeBalance || 0;
      if (totalFee > balance && totalFee >= GATES_FEE_THRESHOLD) {
        // 待扣超过余额 → 暂停开新仓（只保护，不扣费）
        if (!user.gatesFeeLow) {
          this.userDB.set(wallet, { ...user, gatesFeeLow: true, gatesFeeApproved: true });
          this._log(`⚠️ ${wallet.slice(0, 10)}... 待打赏金额 $${totalFee.toFixed(2)} 超过余额 $${balance.toFixed(2)}，暂停开新仓（自愿打赏模式，可主动调用 /api/vault/tip 打赏）`);
        }
      }
    }
    // pending 保留不别除，等用户主动打赏后才清除
  }

  // ═══ 自愿打赏接口：用户主动调用，平台才发起链上转账 ═══
  // 由 /api/vault/tip 调用
  async processTip(wallet, amount) {
    const feeState = this._feeState[wallet];
    if (!feeState || !feeState.pending || feeState.pending.length === 0) {
      return { ok: false, msg: '没有待打赏费用' };
    }
    ...
    // 打赏金额不得超待打赏总额
    const tipAmount = Math.min(parseFloat(amount) || 0, totalFee);
    if (tipAmount <= 0) return { ok: false, msg: '打赏金额无效' };
    ...
    // 按 2:1 比例分给平台 / 生态费（与原盖茨费一致）
    const platformAmount = tipAmount * 2 / 3;
    const ecoAmount = tipAmount * 1 / 3;
    const txs = [];

    try {
      if (platformAmount > 0) {
        const platformWei = ethers.parseUnits(platformAmount.toFixed(6), 18);
        this._log(`💰 ${wallet.slice(0, 10)}... 用户自愿打赏服务费 $${platformAmount.toFixed(2)} → ${PLATFORM_WALLET.slice(0, 10)}...`);
        const tx1 = await usdtContract.transfer(PLATFORM_WALLET, platformWei);
        await tx1.wait();
        txs.push({ type: 'platform', tx: tx1.hash, amount: platformAmount });
      }
      if (ecoAmount > 0) {
        const ecoWei = ethers.parseUnits(ecoAmount.toFixed(6), 18);
        const tx2 = await usdtContract.transfer(ECO_FUND_WALLET, ecoWei);
        await tx2.wait();
        txs.push({ type: 'ecoFund', tx: tx2.hash, amount: ecoAmount });
      }

      // 清除 pending（部分打赏则按比例减少）
      const ratio = tipAmount / totalFee;
      if (ratio >= 0.99) {
        feeState.pending = [];
      } else {
        pending.forEach(r => {
          r.platformFee = (parseFloat(r.platformFee) * (1 - ratio)).toFixed(6);
          r.ecoFund = (parseFloat(r.ecoFund) * (1 - ratio)).toFixed(6);
        });
      }
      feeState.collected = (feeState.collected || 0) + tipAmount;

      // 更新用户余额
      if (this.userDB) {
        const user = this.userDB.get(wallet) || {};
        const newBalance = Math.max(0, (user.gatesFeeBalance || 0) - tipAmount);
        this.userDB.set(wallet, {
          ...user,
          gatesFeeBalance: newBalance,
          gatesFeeLow: newBalance < 5,
          gatesFeeCollected: (user.gatesFeeCollected || 0) + tipAmount,
          gatesFeeApproved: true,
        });
      }

      this._log(`✅ ${wallet.slice(0, 10)}... 自愿打赏成功 $${tipAmount.toFixed(2)} (服务费$${platformAmount.toFixed(2)} + 生态费$${ecoAmount.toFixed(2)})`);
      return { ok: true, tipAmount, txs, msg: '打赏成功' };
    } catch (e) {
      this._log(`❌ ${wallet.slice(0, 10)}... 自愿打赏失败: ${e.message?.slice(0, 100)}`);
      return { ok: false, msg: e.message };
    }
  }

  // ═══ 盖茨费扣取（已废弃，自愿打赏模式不再自动扣费） ═══
  // 保留函数避免其他模块调用出错，但内部已不再执行任何链上转账
  // 用户需主动调用 /api/vault/tip 接口发起打赏
