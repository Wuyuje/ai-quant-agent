...
class BBStrategyManager {
  ...
  async _ensureEngine(wallet, userData) {
    ...
    if (engine) {
      ...
      if (engine.running) {
        ...
        // 同步BSC钱包地址和userDB（用于链上transferFrom扣算力 Token）
    ...
    // BSC钱包地址和userDB — 用于链上transferFrom扣算力 Token
    engine.bscWalletAddr = userData.bscWalletAddr || wallet;
    engine.userDB = this.userDB;
  ...
  async _checkGatesFeeBalance(bbUsers) {
    ...
    for (const [wallet, u] of bbUsers) {
      ...
      if (oldLow && !newLow) {
        this._log(`✅ ${wallet.slice(0,10)}... 算力 Token记账余额已充足 $${balance.toFixed(2)}，恢复交易`);
      } else if (!oldLow && newLow) {
        this._log(`⚠️ ${wallet.slice(0,10)}... 算力 Token记账余额不足 $${balance.toFixed(2)} < $${GATES_FEE_THRESHOLD}，暂停交易（请充值到Trader钱包）`);
      }
    }
  }
