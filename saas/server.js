  ...
  _setupRoutes() {
    ...
    const dashboardHandler = async (req, res) => {
      ...
      res.json({
        ...
        user: {
          ...
          // v125: 算力费余额同时复制到 user 根下，兼容前端 go.html 的 u.gatesFeeBalance 读取
    ...
    this.app.post('/api/gates-fee/confirm-recharge', async (req, res) => {
      ...
      try {
        ...
        clearTimeout(_to);
        const _json = await _resp.json();
        const traderBalance = _json.result ? Number(BigInt(_json.result)) / 1e18 : 0;
    ...
    this.app.get('/api/admin/dashboard', adminAuth, async (req, res) => {
      try {
        ...
        res.json({
          ...
          fees: this.userTrader?.getFeeSummary() || { platformWallet: PLATFORM_WALLET, ecoFundWallet: ECO_FUND_WALLET, users: {}, grandTotal: { platformFee: '0', ecoFund: '0' } },
        });
      } catch(e) {
        res.status(500).json({ error: e.message });
      }
